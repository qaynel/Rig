...
G11a** — B1 is locked (GA-2) and Basic/mid/Advanced packaging is deprecated
(GA-9g). Active catalogue design lives in
`business-spec.md`, `../acceptance.md`, and `technical-spec.md`.
**The 2026-07-25 re-grill leaves the S1–S6 freeze intact but reopens every
Advanced/catalogue mechanism in this document. Those passages are subordinate
historical input until the sole Gate-2 authority, `technical-spec.md`, is
amended and re-frozen against the current Gate 1.**
§9 carries the per-slice implementation instructions for S1–S6.
...
5. **Rig stays config, not a runtime.** The host agent is the only brain
   shipped today (#11). G11a is locked **B1** (GA-2); an independent Rig-brain
   runtime remains a non-goal for this delivery band.
...
**S7 — À-la-carte catalogue delivery (UNLOCKED — see advanced Gate 1/2).**
Supersedes the old "Tier 2 Advanced = agents/loops/memory" framing. Active
intent: mandatory agent-tech-safety baseline + user-selected catalogue
(`family → group → service → grade`). B1 holds (no Rig model key / daemon /
memory store). Memory store and Cap A runtime stay Tier 3. Design sources:
`business-spec.md` / `../acceptance.md` and
`technical-spec.md`. Nothing in S1–S6 depends on S7.
...
- **Designing catalogue delivery under an assumed open G11a** — obsolete;
  G11a is locked B1 (GA-2). Do not reopen a Rig-brain runtime without returning
  to grilling.

## 7. Risks and returns to Gate 1

- **R1 — G11a / packaging taxonomy.** Resolved: B1 locked (GA-2); Basic/mid/
  Advanced install packages deprecated (GA-9g). Catalogue delivery is the S7
  path.
