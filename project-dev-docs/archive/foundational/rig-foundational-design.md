...
2. **Install UX = à-la-carte spectrum**, not fixed bundles:
   ...
   - Tier 1: markdown harnesses only (rules + skills + host adapters), no keys/runtime,
     ...
     language in docs only so later delivery has a clean landing zone.
   ...
   - Beyond the markdown bootstrap: **mandatory agent-tech-safety baseline +
     à-la-carte catalogue** (`family → group → service → grade`) — not fixed
     Basic/mid/Advanced packages (those names are deprecated, GA-9g). User keys
     via gitignored `.env` where credentialed surfaces need them; materializer
     + catalogue live here.
   - Tier 3: harness generator (Layer 2), plus Cap A runtime / persistent memory
     store when those are ever in scope.
