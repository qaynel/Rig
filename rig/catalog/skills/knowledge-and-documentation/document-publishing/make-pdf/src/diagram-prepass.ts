    ...
    if (openFence) {
      ...
      if (close && close.char === openFence.char && close.len >= openFence.len && close.info === "") {
        ...
        if (DIAGRAM_LANGS.has(info.lang) && info.render && openFence.indent === 0) {
          ordinal++;
          const token = `rig-diagram-slot-${runId}-${ordinal}`;
          fences.push({
            lang: info.lang,
...
export function buildDiagramFigure(fence: DiagramFence, svg: string): string {
  ...
  const captioned = fence.title
    ...
    : "";
  const pageAttr = fence.page ? ` data-rig-page="${fence.page}"` : "";
  ...
  return [
    `<figure class="diagram" role="img" aria-label="${escapeHtml(label)}"${pageAttr}` +
      ` data-rig-lang="${escapeHtml(fence.lang)}" data-rig-source="${sourceB64}">`,
    cleanSvg,
    captioned,
    `</figure>`,
  ].join("\n");
...
export function decodeFigureSource(figureHtml: string): string | null {
  const m = figureHtml.match(/\bdata-rig-source="([A-Za-z0-9+/=]*)"/);
  if (!m) return null;
  try {
...
export class RenderTab {
  ...
  static open(): RenderTab {
    ...
    const staged = path.join(PAYLOAD_TMP_DIR, `rig-diagram-render-${sha}.html`);
  ...
  private jsViaFile(expression: string): string {
    const file = path.join(
      PAYLOAD_TMP_DIR,
      `rig-diagram-expr-${process.pid}-${crypto.randomBytes(4).toString("hex")}.js`,
    );
...
export function resolveBundlePath(env: NodeJS.ProcessEnv = process.env): string {
  const candidates = [
    ...
    // in the repo and in the ~/.claude/skills/rig global install. argv[0]
    ...
    path.join(os.homedir(), ".claude/skills/rig/lib/diagram-render/dist/diagram-render.html"),
 ...
 * bytes, and annotate the tag with data-rig-px-width/-height for the width
...
export function inlineLocalImages(html: string, opts: PrepassImageOptions): string {
  ...
  return html.replace(IMG_TAG_RE, (tag) => {
    ...
    const dataUri = `data:${mime};base64,${buf.toString("base64")}`;
    const attrs = dims
      ? ` data-rig-px-width="${Math.round(dims.width)}" data-rig-px-height="${Math.round(dims.height)}"`
      : "";
...
function annotateFromDataUri(tag: string, src: string): string {
  try {
    ...
    return tag.replace(
      /^<img\b/i,
      `<img data-rig-px-width="${Math.round(dims.width)}" data-rig-px-height="${Math.round(dims.height)}"`,
    );
  } catch {
    return tag;
  }
