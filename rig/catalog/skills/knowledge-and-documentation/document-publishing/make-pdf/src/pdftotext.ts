 ...
 *   1. $RIG_PDFTOTEXT_BIN env override (preferred, matches v1.24 RIG_*_BIN pattern)
 ...
 *      and guessing causes false positives. Set RIG_PDFTOTEXT_BIN explicitly.
...
export function resolvePdftotext(env: NodeJS.ProcessEnv = process.env): PdftotextInfo {
  // 1 + 2: env overrides (RIG_PDFTOTEXT_BIN preferred, PDFTOTEXT_BIN back-compat).
  const overrideRaw = env.RIG_PDFTOTEXT_BIN ?? env.PDFTOTEXT_BIN;
  ...
  // positives. Windows users set RIG_PDFTOTEXT_BIN explicitly.
  ...
  throw new PdftotextUnavailableError([
    ...
    "",
    "Or set RIG_PDFTOTEXT_BIN to an explicit path:",
    process.platform === "win32"
      ? '  setx RIG_PDFTOTEXT_BIN "C:\\path\\to\\pdftotext.exe"'
      : "  export RIG_PDFTOTEXT_BIN=/path/to/pdftotext",
  ].join("\n"));
}
 ...
 *   1. $RIG_<TOOL>_BIN env override (e.g. RIG_PDFFONTS_BIN)
...
export function resolvePopplerTool(
  ...
  const override = resolveOverride(env[`RIG_${tool.toUpperCase()}_BIN`], env);
