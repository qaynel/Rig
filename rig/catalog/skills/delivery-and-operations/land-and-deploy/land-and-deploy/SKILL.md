...
preamble-tier: 4
version: 1.0.0
description: Land and deploy workflow. (rig)
allowed-tools:
  - Bash
  - Read
  - Write
...
capability: delivery-and-operations.land-and-deploy
guarantees:
  - Merges an approved change and deploys it in one recorded sequence.
  - Stops at the first failing gate instead of continuing.
overlap_tags:
  - deployment
  - land
  - merge
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"land-and-deploy","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.rig/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/rig/bin/rig-timeline-log '{"skill":"land-and-deploy","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/rig/bin/rig-question-log '{"skill":"land-and-deploy","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
eval "$(~/.claude/skills/rig/bin/rig-slug 2>/dev/null)"
if [ ! -f ~/.rig/projects/$SLUG/land-deploy-confirmed ]; then
  echo "FIRST_RUN"
else
  # Check if deploy config has changed since confirmation
  SAVED_HASH=$(cat ~/.rig/projects/$SLUG/land-deploy-confirmed 2>/dev/null)
...
```bash
~/.claude/skills/rig/bin/rig-review-read 2>/dev/null
```
...
```bash
mkdir -p ~/.rig/projects/$SLUG
CURRENT_HASH=$(sed -n '/## Deploy Configuration/,/^## /p' CLAUDE.md 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
WORKFLOW_HASH=$(find .github/workflows -maxdepth 1 \( -name '*deploy*' -o -name '*cd*' \) 2>/dev/null | xargs cat 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
echo "${CURRENT_HASH}-${WORKFLOW_HASH}" > ~/.rig/projects/$SLUG/land-deploy-confirmed
```
Continue to Step 2.
...
```bash
cat ~/.claude/skills/rig/review/checklist.md 2>/dev/null || echo "Checklist not found"
```
...
ls -t ~/.rig-dev/evals/*-e2e-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -20
```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t ~/.rig-dev/evals/*-llm-judge-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -5
```
...
Then run `rig-diff-scope` to classify the changes:

```bash
eval $(~/.claude/skills/rig/bin/rig-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
...
```bash
$B snapshot -i -a -o ".rig/deploy-reports/post-deploy.png"
```
...
Save report to `.rig/deploy-reports/{date}-pr{number}-deploy.md`.
