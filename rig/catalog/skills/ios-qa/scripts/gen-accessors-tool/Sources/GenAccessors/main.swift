...
// ~/.rig/cache/gen-accessors/<hash>/StateAccessor.swift.
...
struct GenAccessors {
    static func main() async {
        ...
        let cacheDir = getEnv("RIG_IOS_CACHE_ROOT")
            ?? ("~/.rig/cache/gen-accessors" as NSString).expandingTildeInPath
    ...
    static func render(specs: [AccessorSpec], buildId: String, accessorHash: String) -> String {
        ...
        if !specs.isEmpty {
            ...
            out += "private enum _RigDebugBridgeSnapshotJSON {\n"
        ...
        for spec in specs {
            ...
            for (index, field) in spec.fields.enumerated() {
                ...
                out += "                }\n"
                out += "                guard let restored\(index): \(typeText) = _RigDebugBridgeSnapshotJSON.decode(raw\(index), as: \(typeText).self) else {\n"
            ...
            for (name, typeText) in spec.fields {
                ...
                out += "            write: { value in\n"
                out += "                guard let typed: \(typeText) = _RigDebugBridgeSnapshotJSON.decode(value, as: \(typeText).self) else { return false }\n"
