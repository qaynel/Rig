...
- **Auth:** Tailscale identity validation via the local `tailscaled` socket
  ...
  `~/.rig/ios-qa-allowlist.json` is the single source of truth for who can
  do what.
...
rig-ios-qa-mint --remote ci@example.com --capability mutate --ttl 1h
...
row to `~/.rig/security/ios-qa-audit.jsonl`:
...
allowlisted, rate limit hit) write to `~/.rig/security/attempts.jsonl`.
...
| `403 identity_not_allowed` | identity missing from allowlist | Owner mint: `rig-ios-qa-mint --remote <identity>` |
