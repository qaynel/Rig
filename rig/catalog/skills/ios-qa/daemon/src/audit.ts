...
export function defaultAuditPath(): string {
  return process.env.RIG_IOS_AUDIT_PATH
    ?? join(homedir(), '.rig', 'security', 'ios-qa-audit.jsonl');
}

export function defaultAttemptsPath(): string {
  return process.env.RIG_IOS_ATTEMPTS_PATH
    ?? join(homedir(), '.rig', 'security', 'attempts.jsonl');
}
...
async function loadDeviceSalt(): Promise<string> {
  ...
  const path = join(homedir(), '.rig', 'security', 'device-salt');
  try {
