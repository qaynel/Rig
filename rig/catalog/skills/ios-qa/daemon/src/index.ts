...
export async function startDaemon(opts: DaemonOptions): Promise<RunningDaemon | { error: string; reason?: string }> {
  ...
  if (opts.tailnetEnabled && probe?.ok) {
    ...
    const tailnetBindAddr = process.env.RIG_IOS_TAILNET_BIND ?? '127.0.0.1';
...
if (import.meta.main) {
  const port = parseInt(process.env.RIG_IOS_DAEMON_PORT ?? '9099', 10);
  const tailnet = process.argv.includes('--tailnet');
  const targetUDID = process.env.RIG_IOS_TARGET_UDID;
  const bundleId = process.env.RIG_IOS_TARGET_BUNDLE_ID ?? 'com.rig.iosqa.fixture';

  // Default tunnelProvider: when RIG_IOS_TARGET_UDID (or a default with
