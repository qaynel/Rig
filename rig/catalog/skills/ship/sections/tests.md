...
`~/.claude/skills/rig/bin/rig-detach` instead — it survives in its own
session, serializes against other worktrees via a machine lock (no API
saturation), and writes a guaranteed `### rig-detach EXIT=<code> ###` sentinel:

```bash
~/.claude/skills/rig/bin/rig-detach --label ship-evals --lock rig-evals --timeout 5400 -- <project eval command>
```
