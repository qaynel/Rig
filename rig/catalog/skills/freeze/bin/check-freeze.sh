...
STATE_DIR="${CLAUDE_PLUGIN_DATA:-$HOME/.rig}"
...
case "$FILE_PATH" in
  ...
  *)
    ...
    # Log hook fire event
    mkdir -p ~/.rig/analytics 2>/dev/null || true
    echo '{"event":"hook_fire","skill":"freeze","pattern":"boundary_deny","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}' >> ~/.rig/analytics/skill-usage.jsonl 2>/dev/null || true
