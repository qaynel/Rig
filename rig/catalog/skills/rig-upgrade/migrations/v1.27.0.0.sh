#!/usr/bin/env bash
# Migration: v1.27.0.0 — rename rig-brain-* → rig-artifacts-*
#
# Phase C of the v1.27.0.0 plan. Hard-rename, no compat shim. Steps:
#   1. gh_repo_renamed       — gh/glab repo rename rig-brain-$USER →
#                              rig-artifacts-$USER (skipped on user opt-out)
#   2. remote_txt_renamed    — mv ~/.rig-brain-remote.txt → artifacts-remote.txt
#   3. config_key_renamed    — rewrite brain_sync_mode → artifacts_sync_mode
#                              in ~/.rig/config.yaml
#   4. claude_md_block_rewritten — find-and-replace any existing Brain
#                              Configuration block that references "Memory sync"
#   5. sources_swapped       — brain sources add new (verify) → remove old
#                              (codex Finding #6: add-before-remove ordering)
#   6. done                  — write touchfile, delete journal
#
# Interruption-safe via journal at ~/.rig/.migrations/v1.27.0.0.journal:
# each step writes its name on success; re-entry resumes from the next un-done
# step. Done touchfile at ~/.rig/.migrations/v1.27.0.0.done.
#
...
URL_BIN="${BIN_DIR}/rig-artifacts-url"

MIGRATION_DIR="${RIG_HOME}/.migrations"
...
OLD_REPO_NAME="rig-brain-${USER_NAME}"
NEW_REPO_NAME="rig-artifacts-${USER_NAME}"
OLD_REMOTE_TXT="${HOME}/.rig-brain-remote.txt"
NEW_REMOTE_TXT="${HOME}/.rig-artifacts-remote.txt"
...
# `/setup-brain --rerun-migration` removes this marker.)
[ -f "$SKIPPED" ] && exit 0
...
#   - presence of ~/.rig-brain-remote.txt (legacy file)
#   - presence of ~/.rig/.git (brain-init ever ran)
HAS_LEGACY_STATE=0
[ -f "$OLD_REMOTE_TXT" ] && HAS_LEGACY_STATE=1
[ -d "$RIG_HOME/.git" ] && HAS_LEGACY_STATE=1

# If nothing to migrate, finalize silently.
if [ "$HAS_LEGACY_STATE" = "0" ]; then
  echo "  [v1.27.0.0] no legacy rig-brain state detected — nothing to migrate." >&2
  touch "$DONE"
...
if [ ! -f "$JOURNAL" ]; then
  ...
  [v1.27.0.0] rig-brain has been renamed to rig-artifacts.
  ...
  This migration will:
    ...
    2. mv ~/.rig-brain-remote.txt → ~/.rig-artifacts-remote.txt
    3. Rename brain_sync_mode → artifacts_sync_mode in ~/.rig/config.yaml
    4. Update any "## Brain Configuration" block in CLAUDE.md
    5. Update brain federated source registration (local CLI mode)
...
EOF
  if [ -t 0 ]; then
    ...
    case "$REPLY" in
      n|N|no|No|NO)
        echo "  Skipping migration. Re-run via /setup-brain --rerun-migration." >&2
        touch "$SKIPPED"
        exit 0
        ;;
      skip|skip-for-now|s)
...
detect_mcp_mode() {
  ...
  if command -v jq >/dev/null 2>&1 && [ -f "$HOME/.claude.json" ]; then
    local t
    t=$(jq -r '.mcpServers.brain.type // .mcpServers.brain.transport // empty' "$HOME/.claude.json" 2>/dev/null)
    case "$t" in
...
# ---------------------------------------------------------------------------
# Step 2: rename ~/.rig-brain-remote.txt → ~/.rig-artifacts-remote.txt
# ---------------------------------------------------------------------------
if ! journal_done "remote_txt_renamed"; then
  echo "  [v1.27.0.0] step 2: rename ~/.rig-brain-remote.txt → ~/.rig-artifacts-remote.txt" >&2
  if [ -f "$OLD_REMOTE_TXT" ] && [ ! -f "$NEW_REMOTE_TXT" ]; then
    # Update the URL inside if the rename happened on the host: replace
    # rig-brain-$USER with rig-artifacts-$USER in the URL.
...
# ---------------------------------------------------------------------------
# Step 3: rename brain_sync_mode → artifacts_sync_mode in config.yaml
# ---------------------------------------------------------------------------
if ! journal_done "config_key_renamed"; then
  echo "  [v1.27.0.0] step 3: rename brain_sync_mode → artifacts_sync_mode in config.yaml" >&2
  CFG="$RIG_HOME/config.yaml"
  if [ -f "$CFG" ]; then
    # Atomic in-place rewrite with a tmpfile.
    TMP=$(mktemp "${CFG}.v1.27.0.0.XXXXXX")
    sed -e 's/^brain_sync_mode:/artifacts_sync_mode:/' \
        -e 's/^brain_sync_mode_prompted:/artifacts_sync_mode_prompted:/' \
        "$CFG" > "$TMP" && mv "$TMP" "$CFG"
...
# ---------------------------------------------------------------------------
# Step 4: rewrite CLAUDE.md "## Brain Configuration" block fields
# ---------------------------------------------------------------------------
if ! journal_done "claude_md_block_rewritten"; then
  echo "  [v1.27.0.0] step 4: rewrite CLAUDE.md Brain Configuration block fields" >&2
  # Look in cwd's CLAUDE.md (where /setup-brain wrote it) and ~/.rig/CLAUDE.md
  # if it exists. We can't know every project's CLAUDE.md; users rerunning
  # /setup-brain in any project will overwrite that block fresh anyway.
  for CMD in "$PWD/CLAUDE.md" "$RIG_HOME/CLAUDE.md"; do
    [ -f "$CMD" ] || continue
    if grep -q "## Brain Configuration" "$CMD"; then
...
# ---------------------------------------------------------------------------
# Step 5: brain sources swap (add-new before remove-old per codex Finding #6)
# ---------------------------------------------------------------------------
if ! journal_done "sources_swapped"; then
  echo "  [v1.27.0.0] step 5: brain federated source rename" >&2
  if [ "$MCP_MODE" = "remote-http" ]; then
    # Print commands for the brain admin; we can't execute them locally.
    cat >&2 <<EOF
    Remote MCP detected. The local brain CLI can't update the brain's
    federated source registration. Send this to your brain admin:

      brain sources add ${NEW_SOURCE_ID} --path <new-clone-path> --federated
      # verify the new source is searching as expected, then:
      brain sources remove ${OLD_SOURCE_ID} --yes

    (Add-new before remove-old keeps search uninterrupted.)

EOF
    mark_done "sources_swapped"
  elif command -v brain >/dev/null 2>&1 && [ -d "$RIG_HOME/.git" ]; then
    ...
    WORKTREE="${RIG_BRAIN_WORKTREE:-$HOME/.rig-brain-worktree}"
    if brain sources list 2>/dev/null | grep -q "$OLD_SOURCE_ID"; then
      if brain sources add "$NEW_SOURCE_ID" --path "$WORKTREE" --federated 2>/dev/null; then
        echo "    added $NEW_SOURCE_ID" >&2
        if brain sources remove "$OLD_SOURCE_ID" --yes 2>/dev/null; then
          echo "    removed $OLD_SOURCE_ID" >&2
        else
          echo "    WARNING: failed to remove $OLD_SOURCE_ID; both registered. Run manually:" >&2
          echo "    brain sources remove $OLD_SOURCE_ID --yes" >&2
        fi
      else
    ...
    echo "    brain CLI not available or no ~/.rig/.git — skipping" >&2
    mark_done "sources_swapped"
  fi
fi
