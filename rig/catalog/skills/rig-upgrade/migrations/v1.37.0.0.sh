#!/usr/bin/env bash
# Migration: v1.37.0.0 — split-engine brain (remote MCP brain + optional
...
# symbol-aware code search (brain code-def / code-refs / code-callers)
# is now available via /setup-brain Step 4.5 if they want it.
#
# When to print the notice (state match — all conditions must hold):
#   - ~/.claude.json declares mcpServers.brain.{type|transport} = http|sse|url
#     OR mcpServers.brain.url is set (remote-http MCP active)
#   - ~/.brain/config.json is absent (no local engine yet)
#   - User has not previously opted out via:
#       ~/.claude/skills/rig/bin/rig-config set local_code_index_offered true
#
# When silent: anything else (Path 1/2/3 users, anyone already on PGLite,
# anyone who opted out, anyone without remote-http MCP).
#
# Idempotency: writes a touchfile at ~/.rig/.migrations/v1.37.0.0.done
...
MIGRATIONS_DIR="$RIG_HOME/.migrations"
DONE_TOUCH="$MIGRATIONS_DIR/v1.37.0.0.done"
CONFIG_BIN="$HOME/.claude/skills/rig/bin/rig-config"
CLAUDE_JSON="$HOME/.claude.json"
BRAIN_CONFIG="$HOME/.brain/config.json"
...
is_remote_http_mcp() {
  ...
  local mtype murl
  mtype=$(jq -r '.mcpServers.brain.type // .mcpServers.brain.transport // empty' "$CLAUDE_JSON" 2>/dev/null)
  murl=$(jq -r '.mcpServers.brain.url // empty' "$CLAUDE_JSON" 2>/dev/null)
  case "$mtype" in
...
if is_remote_http_mcp && is_local_engine_missing; then
  ...
  ┌──────────────────────────────────────────────────────────────────┐
  │  rig v1.37.0.0 — split-engine brain                          │
  │                                                                  │
  │  Symbol-aware code search is now available on this machine.      │
  │  Your remote brain at brain MCP keeps working as today; you can │
  │  add a tiny local PGLite (~30s, no accounts) for `brain         │
  │  code-def` / `code-refs` / `code-callers` queries per worktree.  │
  │                                                                  │
  │  Run /setup-brain to opt in at Step 4.5. Or skip this notice    │
  │  permanently:                                                    │
  │    rig-config set local_code_index_offered true               │
  └──────────────────────────────────────────────────────────────────┘
