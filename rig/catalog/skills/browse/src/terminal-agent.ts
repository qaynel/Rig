/**
 * Terminal Agent — PTY-backed Claude Code terminal for the rig browser
...
const STATE_FILE = process.env.BROWSE_STATE_FILE || path.join(process.env.HOME || '/tmp', '.rig', 'browse.json');
...
const KEEPALIVE_INTERVAL_MS = parseInt(
  process.env.RIG_PTY_KEEPALIVE_INTERVAL_MS || '25000',
  10,
);
...
const RING_BUFFER_MAX_BYTES = parseInt(
  process.env.RIG_PTY_RING_BUFFER_BYTES || `${1024 * 1024}`,
  10,
);
...
const DETACH_WINDOW_MS = parseInt(
  process.env.RIG_PTY_DETACH_WINDOW_MS || '60000',
  10,
);
 ...
 *      (updated continuously by the rig browser extension).
...
function buildTabAwarenessHint(stateDir: string): string {
  ...
  return [
    'You are running inside the rig browser sidebar with live access to the user\'s browser tabs.',
    '',
...
function spawnClaude(cols: number, rows: number, onData: (chunk: Buffer) => void) {
  ...
  // headed-mode browser; BROWSE_NO_AUTOSTART prevents claude's rig
...
function buildServer() {
  return Bun.serve({
    ...
    fetch(req, server) {
      ...
      //         - Cookie rig_pty (works for non-browser callers and
      ...
      if (url.pathname === '/ws') {
        ...
        // with a `rig-pty.` prefix (which we strip). Browsers send a
        ...
        for (const raw of protoHeader.split(',').map(s => s.trim()).filter(Boolean)) {
          const candidate = raw.startsWith('rig-pty.') ? raw.slice('rig-pty.'.length) : raw;
        ...
        // Fallback: Cookie rig_pty (legacy / non-browser callers).
        if (!token) {
...
function main() {
  ...
  // sibling rig sessions. Callers (cli.ts spawn site, server.ts
