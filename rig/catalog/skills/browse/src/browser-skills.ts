 ...
 *   project:  <project>/.rig/browser-skills/<name>/
 *   global:   ~/.rig/browser-skills/<name>/
 *   bundled:  <rig-install>/browser-skills/<name>/   (read-only, ships with rig)
 *
...
export function defaultTierPaths(opts: { projectRoot?: string; home?: string; bundledRoot?: string } = {}): TierPaths {
  ...
  return {
    project: projectRoot ? path.join(projectRoot, '.rig', 'browser-skills') : null,
    global: path.join(home, '.rig', 'browser-skills'),
...
function detectBundledRoot(): string {
  // The browse binary lives at <rig-install>/browse/dist/browse.
  // The bundled browser-skills/ dir is a sibling of browse/ (i.e. <rig-install>/browser-skills/).
  ...
  // browse/src/browser-skills.ts → ../../  (the rig root).
 ...
 *
 * Cannot tombstone bundled skills — they ship with rig and are read-only.
 * To remove a bundled skill, override it with a global/project entry, or
 * remove the file from the rig source tree.
 */
