 ...
 *
 * Piggybacks on ~/.rig/analytics/skill-usage.jsonl pattern (existing
 * rig telemetry). Hostname + aggregate counters only; no body content,
 * no agent text, no command args. Respects the user's telemetry tier
 * setting (off | anonymous | community) via rig-config.
 *
...
function analyticsDir(): string {
  return path.join(rigHome(), 'analytics');
}
...
function isDisabled(): boolean {
  ...
  if (process.env.RIG_TELEMETRY_OFF === '1') {
    telemetryDisabled = true;
    return true;
  }
  // Conservative default: telemetry ON unless explicitly off. Users opt out via
  // rig-config set telemetry off (preamble reads this; we trust the env hint).
