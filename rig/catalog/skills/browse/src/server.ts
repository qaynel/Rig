/**
 * rig browse server — persistent Chromium daemon
 *
 * Architecture:
 ...
 *
 * State:
 *   State file: <project-root>/.rig/browse.json (set via BROWSE_STATE_FILE env)
 *   Log files:  <project-root>/.rig/browse-{console,network,dialog}.log
...
export interface ServerConfig {
  ...
  startTime: number;
  /**
   * Overlay hook. Runs AFTER rig resolves auth and BEFORE route dispatch.
   * Invalid tokens are auto-rejected at the rig layer (401 returned
   * before hook fires), so the hook only ever sees valid TokenInfo or null
   * (no token presented). Returning a Response short-circuits rig
   * dispatch; returning null falls through.
   */
  beforeRoute?: (req: Request, surface: Surface, auth: TokenInfo | null) => Promise<Response | null>;
  /**
   * Whether rig owns the lifecycle of the terminal-agent process and its
   ...
   *      matched sibling rig sessions on the same host — see
   ...
   *
   * This is correct for rig's CLI path, which spawns `terminal-agent.ts` as
...
/**
 * Build a ServerConfig-shaped object from process.env. Used by rig's
...
/**
 * Read ngrok authtoken from env var, ~/.rig/ngrok.env, or ngrok's native
...
function resolveNgrokAuthtoken(): string | null {
  ...
  const ngrokEnvPath = path.join(home, '.rig', 'ngrok.env');
...
function generateHelpText(): string {
  ...
  const lines = ['rig browse — headless browser for AI agents', '', 'Commands:'];
...
// so this branch is the normal path for /open-rig-browser.
...
if (BROWSE_PARENT_PID > 0 && !IS_HEADED_WATCHDOG) {
  ...
  setInterval(() => {
    try {
      ...
      //    /pair-agent or /open-rig-browser sessions.
...
// fighting with rig's. CLI path is unchanged.
if (import.meta.main) {
...
export function buildFetchHandler(cfg: ServerConfig): ServerHandle {
  ...
  // to rig-owns. Matches the "default-true preserves CLI bit-for-bit"
  ...
  const AGENT_WATCHDOG_TICK_MS = parseInt(
    process.env.RIG_AGENT_WATCHDOG_TICK_MS || '60000',
    10,
  );
  ...
  async function shutdown(exitCode: number = 0) {
    ...
    if (ownsTerminalAgent) {
      ...
      // terminal-agent\.ts` regex teardown which matched sibling rig
  ...
  // block rig shutdown — defensive symmetry with the safeUnlinkQuiet /
  ...
  const makeFetchHandler = (surface: Surface) => async (req: Request): Promise<Response> => {
    ...
    if (url.pathname === '/connect' && req.method === 'GET') {
      ...
      // Welcome page — served when Rig Browser launches in headed mode
      if (url.pathname === '/welcome') {
        const welcomePath = (() => {
          // Gate RIG_SLUG on a strict regex BEFORE interpolating it into
          // the filesystem path. Without this, a slug like "../../etc/passwd"
          // would resolve to ~/.rig/projects/../../etc/passwd/... — path
          ...
          const rawSlug = process.env.RIG_SLUG || 'unknown';
          const slug = /^[a-z0-9_-]+$/.test(rawSlug) ? rawSlug : 'unknown';
          const homeDir = process.env.HOME || process.env.USERPROFILE || '/tmp';
          const projectWelcome = `${homeDir}/.rig/projects/${slug}/designs/welcome-page-20260331/finalized.html`;
          if (fs.existsSync(projectWelcome)) return projectWelcome;
          // Fallback: built-in welcome page from rig install.  Reject
          // SKILL_ROOT values containing '..' for the same defense-in-depth
          // reason as the RIG_SLUG regex above.  Not exploitable today
          // (env set at install time), but the gate is one check.
          const rawSkillRoot = process.env.RIG_SKILL_ROOT || `${homeDir}/.claude/skills/rig`;
        ...
        return new Response(
          `<!DOCTYPE html><html><head><title>Rig Browser</title>
          ...
          <body><div class="msg"><div class="gold">◈</div><p>Rig Browser ready.</p><p style="font-size:.85em">Waiting for commands from Claude Code.</p></div></body></html>`,
      ...
      // extension's rigInjectToTerminal callers. The extension routes
      ...
      if (url.pathname === '/tunnel/start' && req.method === 'POST') {
        ...
        // 1) Resolve ngrok authtoken from env / .rig / native config
...
export async function start() {
  ...
  if (process.env.BROWSE_TUNNEL === '1') {
    const authtoken = resolveNgrokAuthtoken();
    if (!authtoken) {
      console.error('[browse] BROWSE_TUNNEL=1 but no NGROK_AUTHTOKEN found. Set it via env var or ~/.rig/ngrok.env');
    } else {
