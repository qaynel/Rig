 ...
 *
 * Cross-process state lives at ~/.rig/security/session-state.json
...
export type LayerName =
  | 'testsavant_content'
  | 'deberta_content'        // opt-in ensemble layer (RIG_SECURITY_ENSEMBLE=deberta)
...
const SECURITY_DIR = path.join(os.homedir(), '.rig', 'security');
 ...
 * ~/.rig/security/device-salt (0600). Random per-device, prevents rainbow
...
/**
 * Try to locate the rig-telemetry-log binary. Resolution order matches
 * the existing skill preamble pattern (never relies on PATH — packaged
 * binary layouts can break that).
 *
 * Order:
 *  1. ~/.claude/skills/rig/bin/rig-telemetry-log  (global install)
 *  2. .claude/skills/rig/bin/rig-telemetry-log    (symlinked dev)
 *  3. bin/rig-telemetry-log                          (in-repo dev)
 */
function findTelemetryBinary(): string | null {
  const candidates = [
    path.join(os.homedir(), '.claude', 'skills', 'rig', 'bin', 'rig-telemetry-log'),
    path.resolve(process.cwd(), '.claude', 'skills', 'rig', 'bin', 'rig-telemetry-log'),
    path.resolve(process.cwd(), 'bin', 'rig-telemetry-log'),
  ];
 ...
 * RIG_*_BIN override pattern from `browse/src/claude-bin.ts:resolveClaudeCommand`
 * (introduced in v1.24.0.0 #1252) so users on WSL/MSYS2/non-default Git Bash
 * installs can redirect.
 *
 * Override precedence:
 *   1. RIG_BASH_BIN (or BASH_BIN) — absolute path or PATH-resolvable command.
...
export function resolveBashBinary(env: NodeJS.ProcessEnv = process.env): string | null {
  ...
  const override = (env.RIG_BASH_BIN ?? env.BASH_BIN)?.trim();
  if (override) {
 ...
 * Win32: wraps in bash explicitly. `rig-telemetry-log` is a shell script
 ...
 * Bun.which + RIG_*_BIN override pattern as claude-bin.ts.
 *
 * Returns null when bash can't be resolved on Windows (rare — Git Bash ships
 * with the standard rig install path). Caller skips spawn; the local
...
/**
 * Fire-and-forget subprocess invocation of rig-telemetry-log with the
 ...
 * rig-telemetry-log (which respects the user's telemetry tier setting).
...
// per-tab file under ~/.rig/security/decisions/, sidebar-agent polls
