 ...
 * and passes it via RIG_SKILL_TOKEN. The skill never sees the daemon root
 ...
 *    RIG_PORT and RIG_SKILL_TOKEN are always set.
 ...
 * Includes: minimal PATH, locale, terminal type. Skills get RIG_PORT +
 * RIG_SKILL_TOKEN injected separately.
 */
...
export function buildSpawnEnv(opts: BuildEnvOptions): Record<string, string> {
  ...
  if (opts.trusted) {
    ...
    for (const [k, v] of Object.entries(process.env)) {
      ...
      if (k === 'RIG_TOKEN') continue; // never propagate root token
      out[k] = v;
    }
  ...
  // intentionally — e.g. an internal-tool skill — but we still strip RIG_TOKEN
  // above.)
  if (!opts.trusted) {
  ...
  out.RIG_PORT = String(opts.port);
  out.RIG_SKILL_TOKEN = opts.skillToken;
