/**
 * find-browse — locate the rig browse binary.
 *
...
export function locateBinary(): string | null {
  ...
  if (root) {
    for (const m of markers) {
      const local = join(root, m, 'skills', 'rig', 'browse', 'dist', 'browse');
    ...
    // - rig repo dev workflow before `./setup` runs
  ...
  // Global fallback
  for (const m of markers) {
    const global = join(home, m, 'skills', 'rig', 'browse', 'dist', 'browse');
