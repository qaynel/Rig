 ...
 * RIG_STEALTH=extended, off by default) DOES fake plugins — that mode
 ...
 *      detectors flag.
 *   4. Report per-install hardware values via RIG_HW_CONCURRENCY /
 *      RIG_DEVICE_MEMORY env vars (set by gbd at startup via
 ...
 * on macOS), passed through the RIG_* env vars. Each field falls
...
// the UA-CH cmdline switch in buildRigLaunchArgs (which reads RIG_PLATFORM
...
export function readHostProfile(): HostProfile {
  ...
  const concurrency = Number(env.RIG_HW_CONCURRENCY);
  const memory = Number(env.RIG_DEVICE_MEMORY);
  return {
...
export function buildStealthScript(hw: HostProfile): string {
  ...
  // ──── Per-install hardware values from RIG_* env (T2) ────
...
function extendedModeEnabled(): boolean {
  const v = process.env.RIG_STEALTH;
 ...
 *   3. EXTENDED_STEALTH_SCRIPT — only when RIG_STEALTH=extended (off by
 ...
 * disagree with the env-driven GPU spoof in buildRigLaunchArgs on non-Apple
...
/**
 * Build the `--rig-*=` cmdline switches that the Pack 1 Chromium
 * patches read (webgl-vendor-spoof, ua-client-hints-stealth, worker-
 * navigator-stealth). Values come from the RIG_* env vars that
 ...
 *
 * TRUSTED-SOURCE ONLY. These RIG_* values are populated by gbd from
 ...
 *   RIG_GPU_VENDOR        → --rig-gpu-vendor        → webgl-vendor-spoof.patch
 *   RIG_GPU_RENDERER      → --rig-gpu-renderer      → webgl-vendor-spoof.patch
 *   RIG_PLATFORM          → --rig-ua-platform       → ua-client-hints-stealth.patch
 *                              (maps MacARM/MacIntel → "macOS")
 *   RIG_GPU_CHIPSET       → --rig-ua-model          → ua-client-hints-stealth.patch
 *   RIG_HW_CONCURRENCY    → --rig-hw-concurrency    → worker-navigator-stealth.patch
 *   RIG_DEVICE_MEMORY     → --rig-device-memory     → worker-navigator-stealth.patch
 */
export function buildRigLaunchArgs(): string[] {
  const env = (globalThis as any).process?.env ?? {};
  const args: string[] = [];

  const vendor = env.RIG_GPU_VENDOR;
  if (vendor) args.push(`--rig-gpu-vendor=${vendor}`);

  const renderer = env.RIG_GPU_RENDERER;
  if (renderer) args.push(`--rig-gpu-renderer=${renderer}`);
  ...
  const platform = env.RIG_PLATFORM;
  if (platform === 'MacARM' || platform === 'MacIntel') {
    args.push('--rig-ua-platform=macOS');
  } else if (platform === 'Win32') {
    args.push('--rig-ua-platform=Windows');
  } else if (platform && platform.startsWith('Linux')) {
    args.push('--rig-ua-platform=Linux');
  }

  const chipset = env.RIG_GPU_CHIPSET;
  if (chipset) args.push(`--rig-ua-model=${chipset}`);

  const hw = env.RIG_HW_CONCURRENCY;
  if (hw) args.push(`--rig-hw-concurrency=${hw}`);

  const memory = env.RIG_DEVICE_MEMORY;
  if (memory) args.push(`--rig-device-memory=${memory}`);
  ...
  //
  // OPT-IN (off by default): only emitted when RIG_CDP_STEALTH is
  // on/1/true. This switch is read by a C++ patch that only exists in
  // gbrowser builds; gbd opts in by exporting RIG_CDP_STEALTH=on. Stock
  ...
  // unless RIG_CDP_STEALTH=off, which contradicted this very comment.)
  const cdpStealth = env.RIG_CDP_STEALTH;
  if (cdpStealth === 'on' || cdpStealth === '1' || cdpStealth === 'true') {
    args.push('--rig-suppress-prepare-stack-trace');
  }
