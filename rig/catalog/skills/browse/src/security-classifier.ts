 ...
 *
 * To upgrade: bump this string, run `RIG_BENCH_ENSEMBLE=1 bun test
 ...
 * layout at ~/.rig/models/testsavant-small/ on first use.
 *
 ...
 */
const MODELS_DIR = path.join(os.homedir(), '.rig', 'models');
...
// RIG_SECURITY_ENSEMBLE=deberta. Not forced on every install because
...
function isDebertaEnabled(): boolean {
  const setting = (process.env.RIG_SECURITY_ENSEMBLE ?? '').toLowerCase();
...
export function loadTestsavant(onProgress?: (msg: string) => void): Promise<void> {
  if (process.env.RIG_SECURITY_OFF === '1') {
    testsavantState = 'failed';
    testsavantLoadError = 'RIG_SECURITY_OFF=1 — ML classifier kill switch engaged';
...
export function loadDeberta(onProgress?: (msg: string) => void): Promise<void> {
  if (process.env.RIG_SECURITY_OFF === '1') return Promise.resolve();
...
export async function checkTranscript(params: {
  ...
  return new Promise((resolve) => {
    ...
    // repo with a prompt-injection-defense CLAUDE.md (like rig itself),
    ...
    // Env var RIG_HAIKU_TIMEOUT_MS (milliseconds) overrides for benches
    // that want a different budget.
    const timeoutMs = process.env.RIG_HAIKU_TIMEOUT_MS
