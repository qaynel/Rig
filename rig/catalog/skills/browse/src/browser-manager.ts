...
/**
 * Detect whether RIG_CHROMIUM_PATH points at a custom Chromium build that
 * already bakes the rig extension in as a component extension (e.g.,
 * Rig Browser.app / GBrowser). Passing --load-extension against such a
 * binary triggers a ServiceWorkerState::SetWorkerId DCHECK because two
 * copies of the same service worker try to register.
 *
 * Resolution:
 *   1. RIG_CHROMIUM_KIND === 'custom-extension-baked' (preferred, explicit)
 *   2. RIG_CHROMIUM_PATH path substring contains 'GBrowser' or 'gbrowser'
 *      (fallback for callers that only set the path)
 */
export function isCustomChromium(): boolean {
  if (process.env.RIG_CHROMIUM_KIND === 'custom-extension-baked') return true;
  const p = process.env.RIG_CHROMIUM_PATH || '';
...
export function shouldEnableChromiumSandbox(): boolean {
  ...
  // so /qa hangs without --no-sandbox). Setting RIG_CHROMIUM_NO_SANDBOX=1
  // forces the sandbox off without changing the default for everyone else.
  // See #1562.
  if (process.env.RIG_CHROMIUM_NO_SANDBOX === '1') return false;
...
export async function handleChromiumDisconnect(browser: Browser | null): Promise<void> {
  ...
  console.error('[browse] Console/network logs flushed to .rig/browse-*.log');
  process.exit(1);
}
...
export class BrowserManager {
  ...
  /**
   * Find the rig Chrome extension directory.
  ...
  async launch() {
    ...
    const { STEALTH_LAUNCH_ARGS, buildRigLaunchArgs } = await import('./stealth');
    const launchArgs: string[] = [...STEALTH_LAUNCH_ARGS, ...buildRigLaunchArgs()];
    ...
    if (extensionsDir) {
      ...
      // already bakes the extension in (e.g., GBrowser / Rig Browser.app).
  ...
  // ─── Headed Mode ─────────────────────────────────────────────
  /**
   * Launch Playwright's bundled Chromium in headed mode with the rig
  ...
  async launchHeaded(authToken?: string): Promise<void> {
    ...
    // Find the rig extension directory for auto-loading
    ...
    const launchArgs = [
      ...
      ...STEALTH_LAUNCH_ARGS,
      // Rig Pack 1: per-install hardware/GPU/UA-CH overrides for the
    ...
    if (extensionPath) {
      ...
      // (gbrowser / Rig Browser.app). Loading it twice causes a
      ...
      // the extension is component-baked — it reads ~/.rig/.auth.json at
      // startup to learn how to call the daemon).
      // Write to ~/.rig/.auth.json (not the extension dir, which may be read-only
      // in .app bundles and breaks codesigning).
      if (authToken) {
        const fs = require('fs');
        const path = require('path');
        const rigDir = path.join(process.env.HOME || '/tmp', '.rig');
        mkdirSecure(rigDir);
        const authFile = path.join(rigDir, '.auth.json');
        try {
    ...
    // single-instance CLI check for rig.
    cleanSingletonLocks(userDataDir);

    // Support custom Chromium binary via RIG_CHROMIUM_PATH env var.
    // Used by Rig Browser.app to point at the bundled Chromium.
    const executablePath = process.env.RIG_CHROMIUM_PATH || undefined;

    // Rebrand Chromium → Rig Browser in macOS menu bar / Dock / Cmd+Tab.
      ...
      if (fs.existsSync(chromePlist)) {
        ...
        if (plistContent.includes('Google Chrome for Testing')) {
          const patched = plistContent
            .replace(/Google Chrome for Testing/g, 'Rig Browser');
        ...
        const iconCandidates = [
          ...
          path.join(process.env.HOME || '', '.claude', 'skills', 'rig', 'scripts', 'app', 'icon.icns'), // global install
        ];
    ...
    // the previous "RigBrowser" branding suffix was itself a high-entropy
    ...
    // as RigBrowser" contradiction codex flagged.
    ...
    const indicatorScript = () => {
      const injectIndicator = () => {
        if (document.getElementById('rig-ctrl')) return;
        ...
        topLine.style.cssText = `
          ...
          animation: rig-shimmer 3s linear infinite;
        ...
        style.textContent = `
          ...
          @media (prefers-reduced-motion: reduce) {
            #rig-ctrl { animation: none !important; }
          }
        `;
  ...
  async setDeviceScaleFactor(scale: number, width: number, height: number): Promise<string | null> {
    ...
    if (scale < 1 || scale > 3) {
      throw new Error(`viewport --scale: value must be between 1 and 3 (rig policy cap), got ${scale}`);
    }
    if (this.connectionMode === 'headed') {
  ...
  async handoff(message: string): Promise<string> {
    ...
    try {
      ...
      const launchArgs: string[] = ['--hide-crash-restore-bubble', ...STEALTH_LAUNCH_ARGS, ...buildRigLaunchArgs()];
      if (extensionPath) {
        launchArgs.push(`--disable-extensions-except=${extensionPath}`);
        launchArgs.push(`--load-extension=${extensionPath}`);
      ...
      const userDataDir = path.join(process.env.HOME || '/tmp', '.rig', 'chromium-profile');
      fs.mkdirSync(userDataDir, { recursive: true });
