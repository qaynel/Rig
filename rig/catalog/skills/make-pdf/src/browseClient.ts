 ...
 *   1. $RIG_BROWSE_BIN env override (preferred, matches v1.24 RIG_*_BIN pattern)
 *   2. $BROWSE_BIN env override (back-compat alias)
 *   3. sibling dir: dirname(argv[0])/../browse/dist/browse[.exe]
 *   4. ~/.claude/skills/rig/browse/dist/browse[.exe]
...
export function resolveBrowseBin(env: NodeJS.ProcessEnv = process.env): string {
  // 1 + 2: env overrides (RIG_BROWSE_BIN preferred, BROWSE_BIN back-compat).
  const overrideRaw = env.RIG_BROWSE_BIN ?? env.BROWSE_BIN;
  ...
  const globalPath = path.join(home, ".claude/skills/rig/browse/dist/browse");
  ...
  throw new BrowseClientError(
    ...
    [
      ...
      "",
      "make-pdf needs browse (the rig Chromium daemon) to render PDFs.",
      "Tried:",
      `  - $RIG_BROWSE_BIN (${env.RIG_BROWSE_BIN || "unset"})`,
      `  - $BROWSE_BIN (${env.BROWSE_BIN || "unset"})`,
      `  - sibling: ${siblingCandidates.join(", ")}`,
      `  - global: ${globalPath}`,
      "  - PATH: `browse`",
      "",
      "To fix: run rig setup from the rig repo:",
      "  cd ~/.claude/skills/rig && ./setup",
      "",
      "Or set RIG_BROWSE_BIN explicitly:",
      process.platform === "win32"
        ? '  setx RIG_BROWSE_BIN "C:\\path\\to\\browse.exe"'
        : "  export RIG_BROWSE_BIN=/path/to/browse",
    ].join("\n"),
  );
}
