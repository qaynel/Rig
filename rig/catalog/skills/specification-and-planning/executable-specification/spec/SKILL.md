...
name: spec
version: 0.1.0
description: Turn vague intent into a precise, executable spec in five phases. (rig)
allowed-tools:
  - Bash
  - Read
  - Grep
...
guarantees:
  - Turns vague intent into a bounded, executable specification.
  - Leaves no phase of the specification implicit.
overlap_tags:
  - issue
  - requirements
  - specification
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"spec","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.rig/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/rig/bin/rig-timeline-log '{"skill":"spec","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/rig/bin/rig-question-log '{"skill":"spec","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
bun ~/.claude/skills/rig/lib/redact-audit-log.ts \
...
# Resolve visibility once; cache + reuse. Order: local config (~/.rig, never
# committed) → gh → glab → unknown(=public-strict).
REDACT_VIS=$(~/.claude/skills/rig/bin/rig-config get redact_repo_visibility 2>/dev/null)
...
REDACT_BODY_EOF
REDACT_JSON=$(~/.claude/skills/rig/bin/rig-redact --from-file "$REDACT_FILE" --repo-visibility "$REDACT_VIS" --self-email "$(git config user.email 2>/dev/null)" --json)
REDACT_CODE=$?
```
...
Read `RIG_PLAN_MODE` from the environment (emitted by `## Preamble (run first)
...
3. **No flag, `RIG_PLAN_MODE=active`** → file-only path. Also load the spec
   into the active plan file (specified by `--plan-file <path>` or inferred from
   harness context as the work-to-do).
4. **No flag, `RIG_PLAN_MODE=inactive`** → file + spawn path. The default in
...
reuse it; write the exact bytes to `$REDACT_FILE`; `~/.claude/skills/rig/bin/rig-redact --from-file "$REDACT_FILE"
...
~/.claude/skills/rig/bin/rig-decision-log '{"decision":"Spec filed #ISSUE_NUMBER: TITLE","rationale":"APPROACH","scope":"issue","issue":"ISSUE_NUMBER","source":"skill","confidence":7}' 2>/dev/null || true
```
...
Resolve the archive path via the existing `rig-paths` helper (handles
`RIG_HOME`, `CLAUDE_PLUGIN_DATA`, Windows fallback):

```bash
eval "$(~/.claude/skills/rig/bin/rig-paths)"
eval "$(~/.claude/skills/rig/bin/rig-slug)"
ARCHIVE_DIR="$RIG_STATE_ROOT/projects/$SLUG/specs"
mkdir -p "$ARCHIVE_DIR"
...
spec_plan_mode: ${RIG_PLAN_MODE:-unset}
