...
|----------------|-------------|-----------|-------|
| [screen name]  | ~/.rig/projects/$SLUG/designs/[folder]/[filename].png | [brief description] | [constraints from review] |
```
...
```bash
~/.claude/skills/rig/bin/rig-review-log '{"skill":"plan-design-review","timestamp":"TIMESTAMP","status":"STATUS","initial_score":N,"overall_score":N,"unresolved":N,"decisions_made":N,"commit":"COMMIT"}'
```
...
```bash
~/.claude/skills/rig/bin/rig-learnings-log '{"skill":"plan-design-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
eval "$(~/.claude/skills/rig/bin/rig-slug 2>/dev/null)" 2>/dev/null || true
  ~/.claude/skills/rig/bin/rig-brain-cache invalidate brand --project "$SLUG" 2>/dev/null || true
```
