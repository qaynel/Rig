...
interactive: true
version: 2.0.0
description: Interactive developer experience plan review. (rig)
...
guarantees:
  - Reviews a plan for its effect on developer experience.
  - Names the friction a plan would introduce.
overlap_tags:
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"plan-devex-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.rig/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/rig/bin/rig-timeline-log '{"skill":"plan-devex-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/rig/bin/rig-question-log '{"skill":"plan-devex-review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
\`~/.claude/skills/rig/plan-devex-review/dx-hall-of-fame.md\`
...
eval "$(~/.claude/skills/rig/bin/rig-slug 2>/dev/null)" 2>/dev/null || true
{
  ...
  printf '\n### %s\n\n' "developer-persona"
  ~/.claude/skills/rig/bin/rig-brain-cache get developer-persona --project "$SLUG" 2>/dev/null || printf '_(no developer-persona digest available yet)_\n'
  printf '\n### %s\n\n' "recent-decisions"
  ...
  printf '\n### %s\n\n' "competitive-intel"
  ~/.claude/skills/rig/bin/rig-brain-cache get competitive-intel --project "$SLUG" 2>/dev/null || printf '_(no competitive-intel digest available yet)_\n'
...
Load the "## Pass 1" section from `~/.claude/skills/rig/plan-devex-review/dx-hall-of-fame.md`
...
> **STOP.** Before running the 8 DX passes, required outputs, and review report (only after Step 0 investigation is complete), Read `~/.claude/skills/rig/plan-devex-review/sections/review-sections.md` and execute it
