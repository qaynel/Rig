...
~/.claude/skills/rig/bin/rig-review-read 2>/dev/null | grep plan-devex-review || echo "NO_PRIOR_DX_REVIEWS"
```
...
Load reference: Read the "## Pass 1" section from `~/.claude/skills/rig/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 2" section from `~/.claude/skills/rig/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 3" section from `~/.claude/skills/rig/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 4" section from `~/.claude/skills/rig/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 5" section from `~/.claude/skills/rig/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 6" section from `~/.claude/skills/rig/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 7" section from `~/.claude/skills/rig/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 8" section from `~/.claude/skills/rig/plan-devex-review/dx-hall-of-fame.md`.
...
This is NOT a scored pass. It's a checklist of proven patterns from rig's own DX.
...
(`rig-config set codex_reviews disabled`).
...
- **`disabled`** — the user turned Codex reviews off (`codex_reviews=disabled`). Skip this section entirely; do NOT fall back to a Claude subagent — disabled means no extra review step. Print: "Codex review skipped (codex_reviews disabled). Re-enable: `rig-config set codex_reviews enabled`."
...
stays discoverable: "Running the outside voice automatically (standard step). Disable: `rig-config set codex_reviews disabled`."
...
**Persist the result:**
```bash
~/.claude/skills/rig/bin/rig-review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
Persist after the DX Scorecard — the dashboard, the RIG REVIEW REPORT, and the EXIT
PLAN MODE GATE's "review log was called" check depend on it. **PLAN MODE EXCEPTION — ALWAYS RUN** (writes to `~/.rig/`, not project files):

```bash
~/.claude/skills/rig/bin/rig-review-log '{"skill":"plan-devex-review","timestamp":"TIMESTAMP","status":"STATUS","initial_score":N,"overall_score":N,"product_type":"PRODUCT_TYPE","tthw_current":"TTHW_CURRENT","tthw_target":"TTHW_TARGET","mode":"MODE","persona":"PERSONA","competitive_tier":"COMPETITIVE_TIER","unresolved":N,"commit":"COMMIT"}'
```
...
```bash
~/.claude/skills/rig/bin/rig-learnings-log '{"skill":"plan-devex-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
1. Brain trust policy for the active endpoint is `personal` (check via
   `~/.claude/skills/rig/bin/rig-config get brain_trust_policy@<endpoint-hash>`).
...
2. Feature flag `BRAIN_CALIBRATION_WRITEBACK` is set (today: false; flips
   to true when upstream brain v0.42+ ships `takes_add` MCP op).

When both gates pass, the write-back path uses `mcp__brain__takes_add`
to record a take with weight 0.6 (per SKILL_CALIBRATION_WEIGHTS).
If the MCP op is unavailable, fall back to `mcp__brain__put_page` with
a rig:takes fence block (documented but uglier path).
...
eval "$(~/.claude/skills/rig/bin/rig-slug 2>/dev/null)" 2>/dev/null || true
  ~/.claude/skills/rig/bin/rig-brain-cache invalidate developer-persona --project "$SLUG" 2>/dev/null || true
```
...
```bash
eval "$(~/.claude/skills/rig/bin/rig-slug 2>/dev/null)" 2>/dev/null || true
(~/.claude/skills/rig/bin/rig-brain-cache refresh --project "$SLUG" 2>/dev/null &) || true
```
