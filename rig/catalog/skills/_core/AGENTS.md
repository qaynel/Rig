# rig — AI Engineering Workflow

rig is a collection of SKILL.md files that give AI agents structured roles for
software development. Each skill is a specialist: CEO reviewer, eng manager,
designer, QA lead, release engineer, debugger, and more.

## Available skills

Skills live in `.agents/skills/` (or `~/.claude/skills/rig/` on Claude Code).
...
| `/rig-upgrade` | Update rig to the latest version. |
...
| `/learn` | Manage what rig learned across sessions. |
...
| `/setup-brain` | Set up brain for cross-machine session memory sync. |
| `/sync-brain` | Keep brain current with this repo's code; refresh agent search guidance in CLAUDE.md. |
...
| `/open-rig-browser` | Launch the visible Rig Browser with sidebar + stealth. |
...
|---------|-------------|
| `rig-ios-qa-daemon` | Mac-side broker. Loopback by default; `--tailnet` adds a Tailscale-facing listener with capability tiers and audit logging. |
| `rig-ios-qa-mint` | Owner-grant CLI for the tailnet allowlist (`grant`/`revoke`/`list`). |
| `rig-ios-qa-regen` | Regenerate the canonical local DebugBridge package and typed accessors (`--app-source` / `--bridge-dir`). |

End-to-end walkthrough: [docs/howto-ios-testing-with-rig.md](docs/howto-ios-testing-with-rig.md).
...
- **Windows**: curated Windows-safe subset runs on `windows-latest` via the
  ...
  MSYS today; native PowerShell support is a future expansion. The `bin/rig-paths`
  helper resolves state roots through `CLAUDE_PLUGIN_DATA` / `RIG_HOME` so plugin
...
- State paths resolve via `bin/rig-paths` (sourced via `eval "$(...)"`). Honors `RIG_HOME`, `CLAUDE_PLUGIN_DATA`, `CLAUDE_PLANS_DIR`.
- The `claude` CLI binary resolves via `browse/src/claude-bin.ts` (`Bun.which()` + `RIG_CLAUDE_BIN` override). Set `RIG_CLAUDE_BIN=wsl` plus `RIG_CLAUDE_BIN_ARGS='["claude"]'` to run Claude through WSL on Windows.
