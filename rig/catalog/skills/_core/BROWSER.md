...
rig's browser surface in one document. Headless Chromium daemon, ~70+
...
If you've used rig in the last release or two, the productivity loop is the
...
B=./browse/dist/browse           # or ~/.claude/skills/rig/browse/dist/browse
...
/skillify                        # writes ~/.rig/browser-skills/hn-front/...
...
- **Headed via `$B connect`**. Same daemon, but Chromium is visible (rebranded
  as "Rig Browser") with the Side Panel extension auto-loaded. You watch
...
The shipped headline of v1.19.0.0. Two rig skills wrap the browser-skills
...
- **D3 — Atomic write.** Stages everything to `~/.rig/.tmp/skillify-<spawnId>/`,
...
1. **First call.** CLI checks `<project>/.rig/browse.json` for a running
...
collisions. State at `<project>/.rig/browse.json`.

| Workspace | State file | Port |
|-----------|-----------|------|
| `/code/project-a` | `/code/project-a/.rig/browse.json` | random (10000–60000) |
| `/code/project-b` | `/code/project-b/.rig/browse.json` | random (10000–60000) |
...
| `connect` | Launch headed Rig Browser with Side Panel extension |
...
| Tier | Path | When |
|------|------|------|
| **Project** | `<project>/.rig/browser-skills/<name>/` | Project-specific skills (committed or gitignored) |
| **Global** | `~/.rig/browser-skills/<name>/` | Per-user skills, all projects |
| **Bundled** | `<rig-install>/browser-skills/<name>/` | Ships with rig, read-only |
...
| **Process-side env** | `trusted: true` frontmatter passes `process.env` minus `RIG_TOKEN`. `trusted: false` (default) drops everything except a minimal allowlist (LANG, LC_ALL, TERM, TZ) and pattern-strips secrets (TOKEN/KEY/SECRET/PASSWORD, AWS_*, ANTHROPIC_*, OPENAI_*, GITHUB_*, etc.) | Untrusted (must opt in) |

`RIG_PORT` and `RIG_SKILL_TOKEN` are injected last, so a parent process
can't override them.
...
- `stageSkill(opts)` — writes files to `~/.rig/.tmp/skillify-<spawnId>/<name>/`
...
Storage:
- Per-project: `<project>/.rig/domain-skills/<host>.md`
- Global: `~/.rig/domain-skills/<host>.md`
...
`$B connect` launches **Rig Browser** — a rebranded Chromium controlled by
...
```bash
$B connect              # launches Rig Browser, headed
...
"rig" pill in the bottom-right corner so you always know which Chrome
window is being controlled.

### What "Rig Browser" means
...
rig extension pre-loaded via `launchPersistentContext`. The UA no longer
carries a `RigBrowser` suffix — that branding string was itself a
...
`RIG_STEALTH=extended` (also accepts `1` or `true`; off by default) layers
...
`RIG_*` host-profile env (GPU vendor/renderer, UA-CH platform/model,
hardware) emits the Pack 1 `--rig-gpu-vendor` / `--rig-gpu-renderer` /
`--rig-ua-platform` / `--rig-ua-model` / `--rig-hw-concurrency` /
`--rig-device-memory` switches that push the GPU/UA-CH/hardware spoof down
to native code, and `RIG_CDP_STEALTH=on` (or `1`/`true`) emits the Pack 2
`--rig-suppress-prepare-stack-trace` switch (closes the Cloudflare
...
The Chrome extension that ships baked into Rig Browser shows a live
...
pipe text into the live Claude PTY via `window.rigInjectToTerminal(text)`,
...
that accepts the Bearer token OR the HttpOnly `rig_sse` session cookie
...
| Content script | `extension/content.js` | Page overlays, `rig` pill |
...
```bash
bin/rig-extension    # opens chrome://extensions, copies path to clipboard
```

Or do it manually: `chrome://extensions` → toggle Developer mode → Load
unpacked → navigate to `~/.claude/skills/rig/extension` → pin the
...
HttpOnly `rig_sse` cookie (never valid against `/command`).
...
`~/.rig/security/attempts.jsonl` — append-only, salted SHA-256 of source
+ domain only (no raw IP, no full request body), rotates at 10MB with 5
generations. Per-device salt at `~/.rig/security/device-salt` (mode 0600).
...
The root token is written to `<project>/.rig/browse.json` with chmod 600.
...
token OR a 30-minute HttpOnly `rig_sse` cookie minted via
...
The Terminal pane uses a separate session cookie, `rig_pty`, minted via
...
- `RIG_SECURITY_OFF=1` — emergency kill switch. Classifier stays off
...
- Classifier model cache: `~/.rig/models/testsavant-small/` (112MB, first
  run only) plus `~/.rig/models/deberta-v3-injection/` (721MB, only when
  ensemble enabled).
- Attack log: `~/.rig/security/attempts.jsonl` (salted SHA-256 + domain
  only, rotates at 10MB, 5 generations).
- Per-device salt: `~/.rig/security/device-salt` (0600).
- Session state: `~/.rig/security/session-state.json` (cross-process,
  atomic).
...
(override via `RIG_BROWSE_MAX_HTML_BYTES`).
...
- Console: `.rig/browse-console.log`
- Network: `.rig/browse-network.log`
- Dialog: `.rig/browse-dialog.log`
...
```bash
$B state save my-session         # save cookies + URLs to .rig/browse-state-my-session.json
...
| **rig browse** | **~3s** | **~100-200ms** | **0 tokens** (plain text stdout) |
| **rig browse + codified skill** | **~3s** | **~200ms** | **0 tokens** (single skill invocation) |

In a 20-command browser session, MCP tools burn 30,000–40,000 tokens on
protocol framing alone. rig burns zero. The codified-skill path takes a
...
rig skips all of this. Compiled binary. Plain text in, plain text out.
...
project-tier skill at `/code/project-a/.rig/browser-skills/foo/` shadows
the global `~/.rig/browser-skills/foo/` only inside project-a.
...
| `BROWSE_STATE_FILE` | `.rig/browse.json` | Path to state file |
...
| `RIG_BROWSE_MAX_HTML_BYTES` | 52428800 (50MB) | `load-html` size cap |
| `RIG_SECURITY_OFF` | unset | Emergency kill switch — disable ML classifier |
| `RIG_SECURITY_ENSEMBLE` | unset | Set to `deberta` for 3-classifier ensemble (721MB download) |
| `RIG_STEALTH` | unset | Set to `extended` (also accepts `1`/`true`) to layer six aggressive patches (WebGL spoof, faked plugins, mediaDevices) on top of Layer C. Actively lies; can break sites. |
| `RIG_CDP_STEALTH` | unset | Set to `on`/`1`/`true` to emit `--rig-suppress-prepare-stack-trace` (gbrowser Pack 2 / B11 C++ patch only; no-op on stock Chromium) |
| `RIG_GPU_VENDOR`, `RIG_GPU_RENDERER`, `RIG_GPU_CHIPSET` | unset | Per-install GPU spoof fed to the Pack 1 WebGL/UA-CH C++ patches. Set by gbd from the host profile; emitted as `--rig-gpu-vendor` / `--rig-gpu-renderer` / `--rig-ua-model` cmdline switches only when present. |
| `RIG_PLATFORM` | unset | Host platform classification (`MacARM`/`MacIntel` → `macOS`, `Win32` → `Windows`, `Linux*` → `Linux`) emitted as `--rig-ua-platform` |
| `RIG_HW_CONCURRENCY`, `RIG_DEVICE_MEMORY` | host profile (fallback 8) | Per-install `hardwareConcurrency`/`deviceMemory` reported by Layer C and emitted as `--rig-hw-concurrency` / `--rig-device-memory` for the worker-navigator C++ patch |
...
│   ├── stealth.ts               # Layer C: webdriver mask + window.chrome.* + Notification/Permissions + per-install hardware + toString proxy + automation-global sweep; buildRigLaunchArgs (RIG_* cmdline switches); RIG_STEALTH=extended opt-in
...
│   ├── tunnel-denial-log.ts     # ~/.rig/security/attempts.jsonl writer (salted)
...
scrape/SKILL.md.tmpl             # /scrape rig skill — match-or-prototype entry point
skillify/SKILL.md.tmpl           # /skillify rig skill — codify last /scrape into permanent skill
```
...
lands at `~/.rig/browser-skills/<name>/` after the test passes.
