...
bun run eval:list    # list all eval runs from ~/.rig-dev/evals/
...
**Env keys in Conductor workspaces.** The `RIG_*` env-shim (v1.39.2.0+,
`lib/conductor-env-shim.ts`) promotes `RIG_ANTHROPIC_API_KEY` /
`RIG_OPENAI_API_KEY` to their canonical names inside rig's TS binaries.
Tests run through rig entrypoints inherit this promotion automatically.
...
allowlist-scrubbed env (operator `CONDUCTOR_*`, `CLAUDE_*`, `RIG_*`,
`MCP_*`, `BRAIN_*`, and credentials like `GH_TOKEN` never reach children),
a fresh seeded `CLAUDE_CONFIG_DIR` (no operator `~/.claude` CLAUDE.md /
MCP servers / skills), a temp `RIG_HOME`, and `--strict-mcp-config`.
...
(`CONDUCTOR_WORKSPACE_PATH`, per-test `RIG_HOME`) keeps working. Wiring
...
--verbose`). Results are persisted to `~/.rig-dev/evals/` with auto-comparison
...
│   ├── hermes.ts, brain.ts  # Agent runtime hosts
...
│   ├── resolvers/   # Template resolver modules (preamble, design, review, brain, etc.)
...
├── bin/             # CLI utilities (rig-repo-mode, rig-slug, rig-config, etc.)
...
├── open-rig-browser/  # /open-rig-browser skill (launch Rig Browser)
├── connect-chrome/  # symlink → open-rig-browser (backwards compat)
...
│   └── add-host/    # /rig-contrib-add-host skill
...
2. **If missing, AskUserQuestion** — let the user tell you or let rig search the repo
3. **Persist the answer to CLAUDE.md** so we never have to ask again

This applies to test commands, eval commands, deploy commands, and any other
project-specific behavior. The project owns its config; rig reads it.
...
tighter V0 prose set `rig-config set explain_level terse` (binary switch,
...
server must pass `false` so their discovery files survive rig teardown cycles.
...
match) which would kill sibling rig sessions on the same host; the new
...
connection immediately. `Set-Cookie: rig_pty=...` is kept as a
...
PTY via `window.rigInjectToTerminal(text)`, exposed by
...
return 403. SSE endpoints use a 30-minute HttpOnly `rig_sse` cookie minted via
`POST /sse-session` (never valid against `/command`). Tunnel-surface rejections go
to `~/.rig/security/attempts.jsonl` via `tunnel-denial-log.ts`. Before editing
...
for `server.ts`. See `~/.rig/projects/garrytan-rig/ceo-plans/2026-04-19-prompt-injection-guard.md`
...
**Env knobs:**
- `RIG_SECURITY_OFF=1` — emergency kill switch. Classifier stays off even if
  warmed. Canary is still injected; just the ML scan is skipped.
- `RIG_SECURITY_ENSEMBLE=deberta` — opt-in DeBERTa-v3 ensemble. Adds
...
- Classifier model cache: `~/.rig/models/testsavant-small/` (112MB, first run only)
  plus `~/.rig/models/deberta-v3-injection/` (721MB, only when ensemble enabled)
- Attack log: `~/.rig/security/attempts.jsonl` (salted sha256 + domain only,
  rotates at 10MB, 5 generations)
- Per-device salt: `~/.rig/security/device-salt` (0600)
- Session state: `~/.rig/security/session-state.json` (cross-process, atomic)

## Dev symlink awareness

When developing rig, `.claude/skills/rig` may be a symlink back to this
working directory (gitignored). This means skill changes are **live immediately**,
great for rapid iteration, risky during big refactors where half-written skills
could break other Claude Code sessions using rig concurrently.

**Check once per session:** Run `ls -la .claude/skills/rig` to see if it's a
symlink or a real copy. If it's a symlink to your working directory, be aware that:
- Template changes + `bun run gen:skill-docs` immediately affect all rig invocations
- Breaking changes to SKILL.md.tmpl files can break concurrent rig sessions
- During large refactors, remove the symlink (`rm .claude/skills/rig`) so the
  global install at `~/.claude/skills/rig/` is used instead

**Prefix setting:** Setup creates real directories (not symlinks) at the top level
with a SKILL.md symlink inside (e.g., `qa/SKILL.md -> rig/qa/SKILL.md`). This
ensures Claude discovers them as top-level skills, not nested under `rig/`.
Names are either short (`qa`) or namespaced (`rig-qa`), controlled by
`skill_prefix` in `~/.rig/config.yaml`. Pass `--no-prefix` or `--prefix` to
skip the interactive prompt.

**Note:** Vendoring rig into a project's repo is deprecated. Use global install
+ `./setup --team` instead. See README.md for team mode instructions.

**For plan reviews:** When reviewing plans that modify skill templates or the
gen-skill-docs pipeline, consider whether the changes should be tested in isolation
before going live (especially if the user is actively using rig in other windows).
...
migration script to `rig-upgrade/migrations/`. Read CONTRIBUTING.md's "Upgrade
migrations" section for the format and testing requirements. The upgrade skill runs
these automatically after `./setup` during `/rig-upgrade`.
...
direct `gh issue create`, and `RIG_REDACT_PREPUSH=skip` all bypass it. It
...
- **CLI:** `bin/rig-redact` (exit 0 clean / 2 MEDIUM / 3 HIGH; `--json`,
  `--auto-redact`, `--repo-visibility`, `--from-file`). `bin/rig-redact-prepush`
...
- **Visibility (no tier promotion):** resolve once per run, order = local config
  (`rig-config get redact_repo_visibility`, ~/.rig so never committed) → gh
...
- **Audit:** the /spec semantic pass appends a content-free record (categories +
  body sha256, no spec text) to `~/.rig/security/semantic-reviews.jsonl` (0600).
...
When the user says "check out <PR link>" and the PR is from `garrytan-agents/rig`
(or any other fork that is NOT a collaborator on `garrytan/rig`), do NOT just
...
**Workflow:** push the branch to `garrytan/rig` (the base repo) and re-target
the PR from there.
...
2. Push the same branch to the base repo: `git push origin HEAD:<branch-name>`
   (origin = `garrytan/rig`, since the worktree is set up with that remote).
...
`garrytan/rig`. Adding it as a collaborator (option A) or flipping the
...
`bin/rig-next-version` advances within the chosen bump level rather than
...
the Rig/Garry voice, one viewport's worth of prose + tables that lands like a
...
When estimating or discussing effort, always show both human-team and CC+rig time:

| Task type | Human team | CC+rig | Compression |
|-----------|-----------|-----------|-------------|
...
Contributors can store long-range vision docs and design documents in `~/.rig-dev/plans/`.
...
`bin/rig-detach` — NEVER as a plain backgrounded Bash task. A plain background
...
the run can also die to idle-sleep. `rig-detach` fixes both: a fresh session
...
- Use the `eval:bg*` scripts (`eval:bg`, `eval:bg:all`, `eval:bg:gate`,
  `eval:bg:periodic`) — they wrap the eval command in `rig-detach` with the
  machine-wide `rig-evals` lock (concurrent worktrees serialize instead of
  saturating the shared model API), a per-tier watchdog, and a **run-scoped** log
  under `~/.rig-dev/eval-runs/` (no shared-`/tmp` collision). Each prints its
  log path. Or call `rig-detach [--lock NAME] [--timeout SECS] [--label LBL] --
...
- Then **poll the printed logfile** with a death-aware watcher: break on the
  guaranteed `### rig-detach EXIT=<code> ###` sentinel (success AND failure are
...
Native OpenClaw skills live in `openclaw/skills/rig-openclaw-*/SKILL.md`. These are
...
```bash
clawhub publish openclaw/skills/rig-openclaw-office-hours \
  --slug rig-openclaw-office-hours --name "rig Office Hours" \
  --version 1.0.0 --changelog "description of changes"
```

Repeat for each skill: `rig-openclaw-ceo-review`, `rig-openclaw-investigate`,
`rig-openclaw-retro`. Bump `--version` on each update.
...
**Verification:** `clawhub search rig` to confirm they're live.

## Deploying to the active skill

The active skill lives at `~/.claude/skills/rig/`. After making changes:

1. Push your branch
2. Fetch and reset in the skill directory: `cd ~/.claude/skills/rig && git fetch origin && git reset --hard origin/main`
3. Rebuild: `cd ~/.claude/skills/rig && bun run build`

**If you use brain:** the `git reset --hard` in step 2 reverts the brain-aware
(`BRAIN_CONTEXT_LOAD` / `BRAIN_SAVE_RESULTS`) blocks that `rig-config
brain-refresh` renders into the install (those generated blocks differ from
`main` by design). After deploying, re-run `rig-config brain-refresh` to
restore them across all your projects' Claude sessions. It's idempotent.

Or copy the binaries directly:
- `cp browse/dist/browse ~/.claude/skills/rig/browse/dist/browse`
- `cp design/dist/design ~/.claude/skills/rig/design/dist/design`
...
store at `~/.rig/projects/<slug>/decisions.jsonl` so neither you nor the user
re-litigates a settled call or loses the "why" across sessions. This is the reliable,
file-only path: it works with brain OFF. (brain semantic recall is an optional
enhancement layered on top, never a dependency.)

- **Resurface** active decisions before re-deciding: `bin/rig-decision-search`
  (`--recent N`, `--scope repo|branch|issue`, `--query KW`, `--all`, `--json`).
  Add `--semantic` (with `--query`) to append related hits from brain memory when
  it's up; it degrades silently to the reliable file results when brain is off.
...
- **Capture** a DURABLE decision when you or the user make one:
  `bin/rig-decision-log '{"decision":"...","rationale":"...","scope":"repo|branch|issue","source":"user|skill|agent","confidence":1-10}'`.
...
`.brain-source` file in the repo root (kubectl-style context). Any
`brain code-def`, `code-refs`, `code-callers`, `code-callees`, or `query`
