...
//
// Invoked from bin/rig-ios-qa-mint.
...
function printHelp() {
  const help = `rig-ios-qa-mint — manage the tailnet allowlist for remote iOS QA agents

USAGE
  rig-ios-qa-mint grant  --remote <identity> [--capability <tier>] [--ttl <seconds>] [--note <text>]
  rig-ios-qa-mint revoke --remote <identity>
  rig-ios-qa-mint list
...
EXAMPLES
  rig-ios-qa-mint grant --remote 'alice@example.com' --capability interact
  rig-ios-qa-mint grant --remote 'tag:ci' --capability mutate --ttl 86400 --note 'nightly run'
  rig-ios-qa-mint revoke --remote 'alice@example.com'
  rig-ios-qa-mint list

The allowlist lives at ~/.rig/ios-qa-allowlist.json (mode 0600). The daemon's
...
if (import.meta.main) {
  main().catch((err) => {
    process.stderr.write(`rig-ios-qa-mint: ${(err as Error).message}\n`);
    process.exit(1);
  });
}
