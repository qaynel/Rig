...
(adapt to whatever project you're in — these are not rig-specific):
...
**If GitHub:**
```bash
gh pr view --json body -q .body > /tmp/rig-pr-body-$$.md
```

**If GitLab:**
```bash
glab mr view -F json 2>/dev/null | python3 -c "import sys,json; print(json.load(sys.stdin).get('description',''))" > /tmp/rig-pr-body-$$.md
```
...
4. Redaction scan-at-sink, then write the updated body back. The body is already
   in a temp file (`/tmp/rig-pr-body-$$.md`); scan THAT file before editing so
...
~/.claude/skills/rig/bin/rig-redact --from-file /tmp/rig-pr-body-$$.md --repo-visibility "${REDACT_VIS:-unknown}" --json
# exit 3 (HIGH) → do NOT edit, rotate+redact; exit 2 (MEDIUM) → confirm per finding.
```

**If GitHub:**
```bash
gh pr edit --body-file /tmp/rig-pr-body-$$.md
```

**If GitLab:**
Read the contents of `/tmp/rig-pr-body-$$.md` using the Read tool, then pass it to `glab mr update` using a heredoc to avoid shell metacharacter issues:
```bash
...
```bash
NEW_TITLE=$(~/.claude/skills/rig/bin/rig-pr-title-rewrite.sh "$V" "$CURRENT_TITLE")
```
...
stays discoverable: "Running the Codex doc review automatically (standard step). Disable: `rig-config set codex_reviews disabled`."
...
**Persist the result:**
```bash
~/.claude/skills/rig/bin/rig-review-log '{"skill":"codex-doc-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
