 ...
 *
 * /skillify stages a candidate skill into ~/.rig/.tmp/skillify-<spawnId>/,
 ...
 *   <tmpRoot>/.rig/.tmp/skillify-<spawnId>/<name>/
 *
...
export function stageSkill(opts: StageSkillOptions): string {
  ...
  const tmpRoot = opts.tmpRoot ?? path.join(os.homedir(), '.rig', '.tmp');
