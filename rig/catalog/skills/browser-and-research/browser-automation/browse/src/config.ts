 ...
 *   2. git rev-parse --show-toplevel → projectDir/.rig/
...
export function resolveConfig(
  ...
  if (env.BROWSE_STATE_FILE) {
    ...
    projectDir = path.dirname(stateDir); // parent of .rig/
  } else {
    projectDir = getGitRoot() || process.cwd();
    stateDir = path.join(projectDir, '.rig');
...
/**
 * Create the .rig/ state directory if it doesn't exist.
...
export function ensureStateDir(config: BrowseConfig): void {
  ...
  // Ensure .rig/ is in the project's .gitignore
  const gitignorePath = path.join(config.projectDir, '.gitignore');
  try {
    const content = fs.readFileSync(gitignorePath, 'utf-8');
    if (!content.match(/^\.rig\/?$/m)) {
      const separator = content.endsWith('\n') ? '' : '\n';
      fs.appendFileSync(gitignorePath, `${separator}.rig/\n`);
    }
  } catch (err: any) {
    if (err.code !== 'ENOENT') {
 ...
 *   1. RIG_HOME env (explicit override)
 *   2. $HOME/.rig (default)
 */
export function resolverigHome(): string {
  return process.env.RIG_HOME || path.join(os.homedir(), '.rig');
}
