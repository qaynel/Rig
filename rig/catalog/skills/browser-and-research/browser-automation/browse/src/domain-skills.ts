 ...
 *
 * Scope:
 *   - per-project: ~/.rig/projects/<slug>/learnings.jsonl
 *   - global:      ~/.rig/global-domain-skills.jsonl
 *
...
function globalFile(): string {
  return path.join(rigHome(), 'global-domain-skills.jsonl');
}

function projectFile(slug: string): string {
  return path.join(rigHome(), 'projects', slug, 'learnings.jsonl');
}
