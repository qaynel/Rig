...
preamble-tier: 1
version: 1.1.0
description: Fast headless browser for QA testing and site dogfooding. (rig)
triggers:
  - browse a page
  - headless browser
...
tool: rig-browser
capability: browser-and-research.browser-automation
guarantees:
  - Drives a headless browser session without installing a browser into the repository.
  - Returns page state the caller can verify.
overlap_tags:
  - browser
  - browser-automation
  - headless-browser
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"browse","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.rig/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/rig/bin/rig-timeline-log '{"skill":"browse","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```
Scale must be 1-3 (rig policy cap). Changing `--scale` recreates the browser context; refs from `snapshot` are invalidated (rerun `snapshot`), but `load-html` content is replayed automatically. Not supported in headed mode.
...
`$B` is the browse binary (resolved from `$_ROOT/.claude/skills/rig/browse/dist/browse` or `~/.claude/skills/rig/browse/dist/browse`).
