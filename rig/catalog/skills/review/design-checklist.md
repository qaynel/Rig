...
**Trigger:** Only run this checklist if the diff touches frontend files. Use `rig-diff-scope` to detect:

```bash
source <(~/.claude/skills/rig/bin/rig-diff-scope <base> 2>/dev/null)
```
