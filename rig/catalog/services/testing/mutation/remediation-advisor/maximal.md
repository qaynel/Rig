# testing.mutation.remediation-advisor - maximal
...
After the minimal and mid checks pass, `remediation-advisor-evidence-receipt` covers `mutation.remediation-advisor`. Given the mutation.remediation-advisor binding and its inputs are unchanged, it passes only when a second run reproduces the verdict and input/output digests; it fails when the receipt is stale, incomplete, or cannot be rerun. The receipt at `fixture:testing.mutation.remediation-advisor:evidence-replay` names the binding, inputs, outcome, and current digest.
