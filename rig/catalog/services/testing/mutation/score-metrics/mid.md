# testing.mutation.score-metrics - mid
...
After the minimal boundary passes, `score-metrics-mutation-context` evaluates `mutation.score-metrics`. Given mutation.score-metrics has a repository-specific convention and evidence source, it passes only when the result names the consulted path or command and excludes an adjacent mutation concern; it fails when the result infers local convention without an inspectable source. The target is `fixture:testing.mutation.score-metrics:repository-context`, and adjacent mutation ownership stays outside the verdict.
