# testing.mutation.mutant-generator - mid
...
After the minimal boundary passes, `mutant-generator-mutation-context` evaluates `mutation.mutant-generator`. Given mutation.mutant-generator has a repository-specific convention and evidence source, it passes only when the result names the consulted path or command and excludes an adjacent mutation concern; it fails when the result infers local convention without an inspectable source. The target is `fixture:testing.mutation.mutant-generator:repository-context`, and adjacent mutation ownership stays outside the verdict.
