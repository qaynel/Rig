# testing.flaky-reliability.detection-quarantine - mid

Grade: mid (Context target). Until the named local evidence is wired, assurance remains Policy-grade generic baseline practice rather than repository-tailored coverage.

After the minimal boundary passes, `detection-quarantine-flaky-reliability-context` evaluates `flaky-reliability.detection-quarantine`. Given flaky-reliability.detection-quarantine has a repository-specific convention and evidence source, it passes only when the result names the consulted path or command and excludes an adjacent flaky reliability concern; it fails when the result infers local convention without an inspectable source. The target is `fixture:testing.flaky-reliability.detection-quarantine:repository-context`, and adjacent flaky reliability ownership stays outside the verdict.
