# product-security.license-compliance.attribution - mid
...
After the minimal boundary passes, `attribution-license-compliance-context` evaluates `license-compliance.attribution`. Given license-compliance.attribution has a repository-specific convention and evidence source, it passes only when the result names the consulted path or command and excludes an adjacent license compliance concern; it fails when the result infers local convention without an inspectable source. The target is `fixture:product-security.license-compliance.attribution:repository-context`, and adjacent license compliance ownership stays outside the verdict.
