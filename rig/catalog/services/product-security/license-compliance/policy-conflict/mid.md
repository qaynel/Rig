# product-security.license-compliance.policy-conflict - mid
...
After the minimal boundary passes, `policy-conflict-license-compliance-context` evaluates `license-compliance.policy-conflict`. Given license-compliance.policy-conflict has a repository-specific convention and evidence source, it passes only when the result names the consulted path or command and excludes an adjacent license compliance concern; it fails when the result infers local convention without an inspectable source. The target is `fixture:product-security.license-compliance.policy-conflict:repository-context`, and adjacent license compliance ownership stays outside the verdict.
