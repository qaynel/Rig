# infrastructure.cost.rightsizing - maximal
...
After the minimal and mid checks pass, `rightsizing-evidence-receipt` covers `cost.rightsizing`. Given the cost.rightsizing binding and its inputs are unchanged, it passes only when a second run reproduces the verdict and input/output digests; it fails when the receipt is stale, incomplete, or cannot be rerun. The receipt at `fixture:infrastructure.cost.rightsizing:evidence-replay` names the binding, inputs, outcome, and current digest.
