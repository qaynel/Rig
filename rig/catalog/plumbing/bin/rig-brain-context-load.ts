#!/usr/bin/env bun
/**
 * rig-brain-context-load — V1 retrieval surface (Lane C).
 *
 * Called from the rig preamble at every skill start. Reads the active skill's
 * `brain.context_queries:` frontmatter (Layer 2) or falls back to a generic
 * salience block (Layer 1). Dispatches each query by kind:
 *
 *   kind: vector       → brain query <text>
 *   kind: list         → brain list_pages --filter ...
 *   kind: filesystem   → local glob
 *
 * Each MCP/CLI call has a 500ms hard timeout per Section 1C. On timeout or
 * "brain not in PATH" / "MCP not registered", the helper renders
 ...
 *
 * V1.5 P0: salience smarts promote to brain server-side MCP tools
 ...
 *
 * Usage:
 *   rig-brain-context-load --skill office-hours --repo garrytan-rig
 *   rig-brain-context-load --skill-file ./SKILL.md --repo X --user Y
 *   rig-brain-context-load --window 14d --explain
 *   rig-brain-context-load --quiet
 */
...
function printUsage(): void {
  console.error(`Usage: rig-brain-context-load [options]
...
function resolveSkillFile(args: CliArgs): string | null {
  ...
  // Look in common rig skill locations
  const candidates = [
    join(HOME, ".claude", "skills", args.skill, "SKILL.md"),
    join(HOME, ".claude", "skills", "rig", args.skill, "SKILL.md"),
...
function dispatchVector(q: brainManifestQuery, args: CliArgs): QueryResult {
  const t0 = Date.now();
  ...
  if (!brainAvailable()) {
    return { query: q, ok: false, rendered: "", bytes: 0, duration_ms: Date.now() - t0, reason: "brain CLI missing" };
  }
  ...
  if (result.status !== 0 || !result.stdout) {
    return {
      ...
      bytes: 0,
      duration_ms: Date.now() - t0,
      reason: result.error?.message || `brain query exited ${result.status}`,
    };
  }
...
function dispatchList(q: brainManifestQuery, args: CliArgs): QueryResult {
  const t0 = Date.now();
  if (!brainAvailable()) {
    return { query: q, ok: false, rendered: "", bytes: 0, duration_ms: Date.now() - t0, reason: "brain CLI missing" };
  }
  ...
  if (result.status !== 0 || !result.stdout) {
    return {
      ...
      bytes: 0,
      duration_ms: Date.now() - t0,
      reason: result.error?.message || `brain list_pages exited ${result.status}`,
    };
  }
  const rendered = wrapDatamarked(q.render_as, capBody(result.stdout));
  return { query: q, ok: true, rendered, bytes: rendered.length, duration_ms: Date.now() - t0 };
...
function dispatchFilesystem(q: brainManifestQuery, args: CliArgs): QueryResult {
  const t0 = Date.now();
  if (!q.glob) {
...
function capBody(s: string): string {
  ...
  return s.slice(0, PAGE_SIZE_CAP) + `\n\n_(truncated; ${s.length - PAGE_SIZE_CAP} more bytes — query brain directly for full results)_\n`;
}
...
function defaultManifest(args: CliArgs): brainManifest {
...
main().catch((err) => {
  console.error(`rig-brain-context-load fatal: ${err instanceof Error ? err.message : String(err)}`);
  process.exit(1);
});
