#!/usr/bin/env bun
/**
 * rig-brain-sync — V1 unified sync verb.
 *
 * Orchestrates three storage tiers per plan §"Storage tiering":
 *
 *   1. Code (current repo)         → `brain sources add` (idempotent via
 *                                    lib/brain-sources.ts) + `brain sync
 *                                    --strategy code` (incremental) or
 *                                    `brain reindex-code --yes` (--full).
 *                                    NEVER `brain import` (markdown only).
 *   2. Transcripts + curated memory → rig-memory-ingest (typed put_page)
 *   3. Curated artifacts to git    → rig-brain-sync (existing pipeline)
 *
 * Modes:
 ...
 *   - Lock file at ~/.rig/.sync-brain.lock (PID + start ts).
 ...
 *
 * --watch (V1.5 P0 TODO): file-watcher daemon. NOTE: brain v0.25.1 already
 * ships `brain sync --watch [--interval N]` and `brain sync --install-cron`;
 * when revisited, /sync-brain --watch wires through to the brain CLI rather
 * than building a rig-side daemon.
 */
...
import { detectEngineTier, withErrorContext, canonicalizeRemote } from "../lib/rig-memory-helpers";
import { ensureSourceRegistered, sourcePageCount, parseSourcesList, cycleCompleted, type CycleStatus } from "../lib/brain-sources";
import { detectAutopilot, decideSourceRemove, decideCodeSync } from "../lib/brain-guards";
import { localEngineStatus, type LocalEngineStatus } from "../lib/brain-local-status";
import { buildbrainEnv, spawnbrain, execbrainJson, NEEDS_SHELL_ON_WINDOWS } from "../lib/brain-exec";
...
const HOME = homedir();
const RIG_HOME = process.env.RIG_HOME || join(HOME, ".rig");
const STATE_PATH = join(RIG_HOME, ".brain-sync-state.json");
const LOCK_PATH = join(RIG_HOME, ".sync-brain.lock");
 ...
 * RIG_HOME at runtime — same pattern as cacheFilePath() in
 * lib/brain-local-status.ts. Avoids the ESM static-import hoist trap where a
 * module-load-time const captures the real ~/.rig before a test can redirect.
 */
export function dreamMarkerPath(): string {
  return join(process.env.RIG_HOME || join(homedir(), ".rig"), ".dream-in-progress");
}

// Default 35-minute timeout for code-walk + memory-ingest stages. Override via
// RIG_SYNC_CODE_TIMEOUT_MS / RIG_SYNC_MEMORY_TIMEOUT_MS. Bounds-checked
...
/**
 * brain writes ~/.brain/import-checkpoint.json on every import run. If a
 * previous /sync-brain hit the timeout (SIGTERM = exit 143), the checkpoint
 * + its staging dir survive on disk. Detect both and let brain resume from
...
export function readbrainCheckpoint(): brainCheckpoint | null {
  ...
  const cpPath = join(home, ".brain", "import-checkpoint.json");
  if (!existsSync(cpPath)) return null;
  try {
    const raw = readFileSync(cpPath, "utf-8");
    const parsed = JSON.parse(raw);
    if (!parsed || typeof parsed !== "object") return null;
    return parsed as brainCheckpoint;
  } catch {
...
/**
 * Decide whether the next memory-ingest run should resume from brain's
 * checkpoint or restage from scratch.
 *   - no checkpoint              → run a fresh ingest pass
 *   - checkpoint + staging ok    → resume (brain picks up at processedIndex+1)
 *   - checkpoint + staging gone  → warn, fall through to fresh restage
 */
export function decideResume(rigHome: string = RIG_HOME): ResumeVerdict {
  const cp = readbrainCheckpoint();
  if (!cp || !cp.dir) return { kind: "no-checkpoint" };
  const stagingDir = cp.dir;
  // #1802: only resume into a path we can PROVE is a rig-minted staging dir.
  ...
  const verdict = checkOwnedStagingDir(stagingDir, rigHome);
  if (!verdict.ok) {
...
function printUsage(): void {
  console.error(`Usage: rig-brain-sync [--incremental|--full|--dry-run] [options]
...
Options:
  ...
  --no-memory          Skip the rig-memory-ingest stage (transcripts + artifacts).
  --no-brain-sync      Skip the rig-brain-sync git pipeline stage.
  ...
  --dream              Force the source-scoped dream cycle that builds this
                       ...
                       timeout 45min, override RIG_SYNC_DREAM_TIMEOUT_MS.
  ...
  --allow-reclone      Permit the code walk for URL-managed sources (remote_url set)
                       even though brain may auto-reclone the working tree (#1734).
  --help               This text.
 ...
 *
 * Pattern: `rig-code-<slug>-<hostpathhash8>` where slug comes from origin
 ...
 *
 * `RIG_HOSTNAME` env override is honored for deterministic tests; in
 * production paths it is unset and `os.hostname()` is used.
 *
 * brain enforces source ids to be 1-32 lowercase alnum chars with
...
function deriveCodeSourceId(repoPath: string): string {
  const host = process.env.RIG_HOSTNAME || hostname();
  ...
  if (remote) {
    ...
    const fullId = constrainSourceId("rig-code", `${slugSource}-${hostPathHash}`);
    ...
    return constrainSourceId("rig-code", `${repoOnly}-${hostPathHash}`);
  }
  const base = repoPath.split("/").pop() || "repo";
  return constrainSourceId("rig-code", `${base}-${hostPathHash}`);
}

/**
 * Pre-pathhash source id, kept for orphan detection only.
 *
 * Earlier /sync-brain versions registered `rig-code-<slug>` (no pathhash
...
function deriveLegacyCodeSourceId(repoPath: string): string {
  ...
  if (remote) {
    const segs = remote.split("/").filter(Boolean);
    const slugSource = segs.slice(-2).join("-");
    return constrainSourceId("rig-code", slugSource);
  }
  const base = repoPath.split("/").pop() || "repo";
  return constrainSourceId("rig-code", base);
}
 ...
 * repo path: `rig-code-<slug>-<sha1(path).slice(0,8)>`. After #1468 the
 ...
 * detect this form once, attempt rename-in-place if the brain CLI supports
...
export function derivePathOnlyHashLegacyId(repoPath: string): string {
  ...
  if (remote) {
    const segs = remote.split("/").filter(Boolean);
    const slugSource = segs.slice(-2).join("-");
    return constrainSourceId("rig-code", `${slugSource}-${pathHash}`);
  }
  const base = repoPath.split("/").pop() || "repo";
  return constrainSourceId("rig-code", `${base}-${pathHash}`);
}

/**
 * Feature-check whether the installed brain CLI ships `sources rename <old> <new>`.
 *
 * Per the v1.40.0.0 design review: probing `brain sources rename --help` and
 * matching for the exact argument shape catches the case where brain's
 ...
 * of the process. As of brain 0.35.0.0 this command does not exist, so the
 * function returns false and the migration path falls back to register-new
 * + sync-OK + remove-old.
 */
let _brainSupportsRenameCache: boolean | null = null;
export function _resetbrainSupportsRenameCache(): void {
  _brainSupportsRenameCache = null;
}
function brainSupportsSourcesRename(env?: NodeJS.ProcessEnv): boolean {
  if (_brainSupportsRenameCache !== null) return _brainSupportsRenameCache;
  try {
    const r = spawnbrain(["sources", "rename", "--help"], {
      timeout: 5_000,
      baseEnv: env,
    });
    const out = `${r.stdout || ""}\n${r.stderr || ""}`;
    ...
    _brainSupportsRenameCache = exact && r.status === 0;
  } catch {
    _brainSupportsRenameCache = false;
  }
  return _brainSupportsRenameCache;
}

/**
 * Look up a source's `local_path` from `brain sources list --json`.
 * Returns null when the source is absent or the listing fails.
 *
 * `env` is the environment passed to the spawned `brain` process; defaults
 * to `process.env`. Tests inject a PATH that points at a brain shim so the
 * helper can be exercised without a real brain CLI.
 *
 * Shape note: `brain sources list --json` returns `{sources: [...]}` (v0.20+);
 * older versions returned a flat array. Accept both for forward/backward compat
 * (mirrors `probeSource`/`sourcePageCount` in lib/brain-sources.ts).
 */
export function sourceLocalPath(sourceId: string, env?: NodeJS.ProcessEnv): string | null {
  const raw = execbrainJson<unknown>(
    ["sources", "list", "--json"],
    { baseEnv: env },
  );
  if (!raw) return null;
 ...
 *   4. Otherwise: feature-check `brain sources rename`. If supported and the
...
export function planHostnameFoldMigration(
  ...
  if (brainSupportsSourcesRename(env)) {
    const r = spawnbrain(["sources", "rename", legacyPathHashId, newSourceId], { baseEnv: env });
    if (r.status === 0) {
...
/**
 * #1734: run `brain sources remove <id> --confirm-destructive` only behind the
 ...
 * brain-side lease. Refuses when autopilot is active or when the source is
 * user-managed and brain can't keep its storage. Pure side-effect helper; the
  ...
  if (ap.active) {
    return {
      ...
      reason: `autopilot active (${ap.signal}); refusing destructive remove of ${sourceId}. ` +
        `Stop autopilot, then re-run /sync-brain.`,
    };
  }
...
/**
 * Build a brain-valid source id (1-32 lowercase alnum + interior hyphens). Sanitizes
...
function constrainSourceId(prefix: string, raw: string): string {
  ...
  // produce "${prefix}-" which fails brain's validator on the trailing
...
function acquireLock(): boolean {
  mkdirSync(RIG_HOME, { recursive: true });
...
/**
 * Acquire the dream marker (`~/.rig/.dream-in-progress`). Returns false when
 ...
 *   no-cli         → "brain CLI not on PATH; install via /setup-brain"
 *   missing-config → "no local engine; run /setup-brain to add local PGLite"
 *   broken-config  → "config file at ~/.brain/config.json is malformed; see /setup-brain Step 1.5"
 *   broken-db      → "config points at unreachable DB; see /setup-brain Step 1.5"
...
function skipStageForLocalStatus(
  ...
  const reasons: Record<Exclude<LocalEngineStatus, "ok">, string> = {
    "no-cli": "brain CLI not on PATH; install via /setup-brain",
    "missing-config":
      "no local engine; run /setup-brain to add local PGLite for code search",
    "broken-config":
      "config at ~/.brain/config.json is malformed; see /setup-brain Step 1.5",
    "broken-db":
      "config points at unreachable DB; see /setup-brain Step 1.5",
    "engine-locked":
      "PGLite is busy (often held by brain serve); stop the holding process or run /sync-brain outside the live Claude session, then retry",
    "timeout":
      "engine probe timed out; raise RIG_BRAIN_PROBE_TIMEOUT_MS if your pooler is slow",
  };
...
function warnProbeTimeout(stage: "code" | "memory" | "dream"): void {
  process.stderr.write(
    `[rig-brain-sync] ${stage}: engine probe timed out — proceeding anyway; ` +
      `raise RIG_BRAIN_PROBE_TIMEOUT_MS if your pooler is slow\n`,
  );
}
...
async function runCodeImport(args: CliArgs): Promise<StageResult> {
  ...
  // engine state. Useful for "what would /sync-brain do" without probing
  // the engine.
  if (args.mode === "dry-run") {
    return {
      ...
      ok: true,
      duration_ms: 0,
      summary: `would: brain sources add ${sourceId} --path ${root} --federated; brain sync --strategy code --source ${sourceId}; brain sources attach ${sourceId}`,
  ...
  // on local engine. The /sync-brain Step 1.5 pre-flight surfaces the user
  ...
  // Earlier /sync-brain versions registered `rig-code-<slug>` (no path
  ...
  const migration = planHostnameFoldMigration(root, sourceId, pathOnlyHashLegacyId, brainEnv);
  if (migration.kind === "skipped-path-drift" && !args.quiet) {
    console.error(
      ...
      + `Clean up manually with: brain sources remove ${migration.oldId} --confirm-destructive`,
    );
  ...
  try {
    const result = await ensureSourceRegistered(sourceId, root, { federated: true, env: brainEnv });
  ...
  // a full re-embed.
  //
  // `brain reindex-code` only RE-EMBEDS pages that already exist; it never
  ...
  const codeTimeoutMs = resolveStageTimeoutMs(
    process.env.RIG_SYNC_CODE_TIMEOUT_MS,
    "RIG_SYNC_CODE_TIMEOUT_MS",
  );
  ...
  const apBeforeWalk = detectAutopilot(brainEnv);
  if (apBeforeWalk.active) {
    return {
      name: "code", ran: true, ok: false, duration_ms: Date.now() - t0,
      summary: `refused: brain autopilot active (${apBeforeWalk.signal}). Stop autopilot, then re-run /sync-brain.`,
      detail: { source_id: sourceId, source_path: root, status: "refused-autopilot" },
    };
  }
  const reclone = decideCodeSync(sourceId, brainEnv, args.allowReclone);
  if (!reclone.allow) {
    return {
      name: "code", ran: true, ok: false, duration_ms: Date.now() - t0,
  ...
  const walkResult = spawnbrain(["sync", "--strategy", "code", "--source", sourceId], {
  ...
  if (walkResult.status !== 0) {
    return {
      ...
      ok: false,
      duration_ms: Date.now() - t0,
      summary: `brain sync --strategy code --source ${sourceId} exited ${walkResult.status}`,
      detail: { source_id: sourceId, source_path: root, status: "failed" },
    };
  }

  if (args.mode === "full") {
    const reindexResult = spawnbrain(["reindex-code", "--source", sourceId, "--yes"], {
      stdio: args.quiet ? ["ignore", "ignore", "ignore"] : ["ignore", "inherit", "inherit"],
      timeout: codeTimeoutMs,
      baseEnv: brainEnv,
    });
    ...
    if (reindexResult.status !== 0) {
      return {
        ...
        ok: false,
        duration_ms: Date.now() - t0,
        summary: `brain reindex-code --source ${sourceId} exited ${reindexResult.status}`,
        detail: { source_id: sourceId, source_path: root, status: "failed" },
      };
    }
  }

  // Step 3: Pin this worktree's CWD to the source via .brain-source. Subsequent
  // brain code-def / code-refs / code-callers calls from anywhere under <root>
  ...
  // succeeded but unqualified `brain code-def` from this worktree will hit
  ...
  const attach = spawnbrain(["sources", "attach", sourceId], {
    timeout: 10_000,
    cwd: root,
    baseEnv: brainEnv,
  });
  const pageCount = sourcePageCount(sourceId, brainEnv);
  ...
  if (migration.kind === "pending-cleanup" && pageCount !== null && pageCount > 0) {
    hostnameLegacyRemoved = removeOrphanedSource(migration.oldId, brainEnv);
  ...
  if (attach.status !== 0) {
    ...
    return {
      ...
      ok: false,
      duration_ms: Date.now() - t0,
      summary: `${baseSummary}; attach FAILED (${reason}) — code-def queries from this worktree will hit the default source until /sync-brain succeeds`,
      detail: {
        source_id: sourceId,
        source_path: root,
  ...
  // consuming repo, but the change actually only added .brain-source to
  // rig's own .gitignore. Without the consumer-side entry, the pin gets
  ...
  ensurebrainSourceGitignored(root);
...
/**
 * Ensure `.brain-source` is listed in the consumer repo's `.gitignore`.
 *
 ...
 */
export function ensurebrainSourceGitignored(root: string): void {
  ...
  try {
    ...
    const alreadyIgnored = existing
      .split("\n")
      .some((line) => line.trim() === ".brain-source");
    if (alreadyIgnored) {
      return;
    }
    const sep = existing.length > 0 && !existing.endsWith("\n") ? "\n" : "";
    writeFileSync(gitignorePath, existing + sep + ".brain-source\n");
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    console.warn(
      `[sync:code] could not add .brain-source to ${gitignorePath}: ${msg}`,
    );
  }
}
...
function runMemoryIngest(args: CliArgs): StageResult {
  ...
  if (args.mode === "dry-run") {
    return { name: "memory", ran: false, ok: true, duration_ms: 0, summary: "would: rig-memory-ingest --probe" };
  }

  // Split-engine pre-flight (per plan D12). rig-memory-ingest shells out
  // to `brain import` which targets the LOCAL engine. When that engine is
  ...
  // timeout and brain left ~/.brain/import-checkpoint.json plus its staging
  // dir on disk, signal the grandchild via env so it skips the prepare phase
  // and lets `brain import` resume from processedIndex+1 against the same
  ...
  const childEnv = buildbrainEnv({ announce: false });
  if (resume.kind === "resume") {
    console.error(
      `[sync:memory] resuming from brain checkpoint (${resume.processedIndex}/${resume.totalFiles} files staged at ${resume.stagingDir})`,
    );
    childEnv.RIG_INGEST_RESUME_DIR = resume.stagingDir;
    ...
    console.error(
      `[sync:memory] previous checkpoint stale (${why}), restaging from scratch. ` +
        `Remove ~/.brain/import-checkpoint.json to silence.`,
    );
  }

  const ingestPath = join(import.meta.dir, "rig-memory-ingest.ts");
  ...
  // .env.local footgun affects rig-memory-ingest.ts too, not just the
  // direct brain spawns in this file). The grandchild calls brain import
  // internally and must see the DATABASE_URL from brain's own config.
  ...
  // lines indicate a system-level failure (brain crashed or CLI missing)
...
function runBrainSyncPush(args: CliArgs): StageResult {
  ...
  if (args.mode === "dry-run") {
    return { name: "brain-sync", ran: false, ok: true, duration_ms: 0, summary: "would: rig-brain-sync --discover-new --once" };
  }

  const brainSyncPath = join(import.meta.dir, "rig-brain-sync");
  if (!existsSync(brainSyncPath)) {
    return { name: "brain-sync", ran: false, ok: true, duration_ms: 0, summary: "skipped (rig-brain-sync not installed)" };
  }
  ...
  return {
    ...
    ok: result.status === 0,
    duration_ms: Date.now() - t0,
    summary: result.status === 0 ? "curated artifacts pushed" : `rig-brain-sync exited ${result.status}`,
  };
}
 ...
 *     surprise 35-min cycle — see brain-doctor-overstrict).
...
/**
 * Run `brain dream` — the brain-global maintenance cycle whose
...
export async function runDream(args: CliArgs): Promise<StageResult> {
  ...
  if (args.mode === "dry-run") {
    ...
    return {
      ...
      duration_ms: 0,
      summary: sourceId
        ? `would: brain dream --source ${sourceId}  (build this source's call graph)`
        : "would: brain dream  (call-graph build)",
    };
  }
  ...
  try {
    const dreamTimeoutMs = resolveStageTimeoutMs(
      process.env.RIG_SYNC_DREAM_TIMEOUT_MS,
      "RIG_SYNC_DREAM_TIMEOUT_MS",
      DEFAULT_DREAM_TIMEOUT_MS,
    );

    // Scope the cycle to THIS worktree's code source: `brain dream --source <id>`.
    // Verified empirically (not just from `brain --help`): plain `brain dream`
    ...
    // empty. `brain dream --source <id>` runs the per-source cycle (the form
    // `brain doctor` recommends for stale sources) and is what actually populates
    ...
    // spawnbrain seeds DATABASE_URL from brain's config via buildbrainEnv.
    //
    // We CAPTURE output (pipe) rather than inherit because `brain dream` exits 0
    // even when it SKIPS the cycle — when another cycle already holds brain's own
    // DB lock (e.g. a running `brain autopilot`), it prints "Skipped: another
    ...
    if (!args.quiet) {
      process.stderr.write("[dream] running brain cycle (call-graph build; this can take a few minutes)...\n");
    }
    let result: ReturnType<typeof spawnbrain>;
    try {
      result = spawnbrain(dreamArgs, {
      ...
      return {
        ...
        ok: false,
        duration_ms: Date.now() - t0,
        summary: `brain dream failed to start: ${(err as Error).message}`,
      };
    }

    if (result.error) {
      const e = result.error as NodeJS.ErrnoException;
      const why = e.code === "ENOENT" ? "brain not on PATH" : e.message;
      return {
        name: "dream",
        ran: true,
        ok: false,
        duration_ms: Date.now() - t0,
        summary: `brain dream failed to start: ${why}`,
      };
    }
    ...
    if (result.status !== 0) {
      return {
        ...
        ok: false,
        duration_ms: Date.now() - t0,
        summary: `brain dream exited ${result.status === null ? "null (killed by signal / timeout)" : result.status}`,
      };
    }

    // Exit 0 but the cycle was SKIPPED because brain's own lock is held by
    // another cycle (typically `brain autopilot`). Report SKIP, not "built" —
    ...
    if (/already running|\block(?:ed)?\b|Skipped:/i.test(out)) {
      return {
        ...
        ok: true,
        duration_ms: Date.now() - t0,
        summary: "skipped — a brain cycle is already running (e.g. autopilot); the call graph builds on that cycle",
      };
    }

    // Exit 0 and the cycle actually ran. Parse the cycle's OWN output to report
    // the truth, not a flat "built": `brain dream` exits 0 even when the call
    // graph could not be built, and a misleading "built" turns a multi-minute
    // no-op into a silent dead end. brain only surfaces these conditions in the
 ...
 * Returns null when the line is absent (older brain / different pack). The
...
/**
 * Inspect a completed (exit-0) `brain dream` log and return a WARN summary when
...
export function classifyDreamOutcome(out: string): string | null {
  ...
  if (/does not declare this phase/i.test(out)) {
    return (
      ...
      "pack (`brain schema use <pack>`) to enable code-callers/code-callees."
    );
  }
  ...
  if (/embed phase failed/i.test(out) || /requires\s+\S*_API_KEY/i.test(out)) {
    return (
      ...
      "brain process, then re-run /sync-brain --dream."
    );
  }
...
function loadSyncState(): SyncState {
  if (!existsSync(STATE_PATH)) {
    return { schema_version: 1, last_writer: "rig-brain-sync" };
  }
  try {
...
async function main(): Promise<void> {
  ...
  if (!args.quiet) {
    ...
    console.error(`[brain-sync] mode=${args.mode} engine=${engine.engine}`);
  }
  ...
  if (needsLock) {
    ...
    if (!haveLock) {
      console.error(
        `[brain-sync] another /sync-brain is running (lock at ${LOCK_PATH}). ` +
  ...
  try {
    ...
    if (!args.noCode) {
      stages.push(await withErrorContext("sync:code", () => runCodeImport(args), "rig-brain-sync"));
    }
    if (!args.noMemory) {
      stages.push(await withErrorContext("sync:memory", () => runMemoryIngest(args), "rig-brain-sync"));
    }
    if (!args.noBrainSync) {
      stages.push(await withErrorContext("sync:brain-sync", () => runBrainSyncPush(args), "rig-brain-sync"));
    }
    ...
    // long would freeze every other worktree's /sync-brain. Dream is guarded
    // by its own marker.
    cleanup();
  }
  ...
  if (args.mode === "dry-run") {
    ...
    if (shouldRunDream(args, cycle)) {
      ...
      // surprise 35-min run — brain-doctor-overstrict).
      dreamStage = {
        name: "dream",
        ran: false,
        ok: true,
        duration_ms: 0,
        summary: "call-graph state unknown (doctor unavailable) — run /sync-brain --dream if code-callers returns 0",
      };
    }
  }
...
if (import.meta.main) {
  main().catch((err) => {
    console.error(`rig-brain-sync fatal: ${err instanceof Error ? err.message : String(err)}`);
    releaseLock();
    process.exit(1);
  });
}
