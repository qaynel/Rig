#!/usr/bin/env bun
/**
 * rig-memory-ingest — V1 memory ingest helper.
 *
 * Walks coding-agent transcript sources + ~/.rig/ curated artifacts and writes
 * each one to brain as a typed page. Per plan §"Storage tiering": curated memory
 * rides the existing brain Postgres + git pipeline; code/transcripts go to the
 * Supabase tier when configured (or local PGLite otherwise) — never double-store.
 *
 * Usage:
 *   rig-memory-ingest --probe                 # count what would ingest, no writes
 *   rig-memory-ingest --incremental [--quiet] # default; mtime fast-path; cheap
 *   rig-memory-ingest --bulk [--all-history]  # first-run; full walk
 *   rig-memory-ingest --bulk --benchmark      # time the bulk pass + report
 *   rig-memory-ingest --include-unattributed  # also ingest sessions with no git remote
 *
 * Sources walked:
 ...
 *   ~/.rig/projects/<slug>/learnings.jsonl       — typed: learning
 *   ~/.rig/projects/<slug>/timeline.jsonl        — typed: timeline
 *   ~/.rig/projects/<slug>/ceo-plans/*.md        — typed: ceo-plan
 *   ~/.rig/projects/<slug>/*-design-*.md         — typed: design-doc
 *   ~/.rig/analytics/eureka.jsonl                — typed: eureka
 *   ~/.rig/builder-profile.jsonl                 — typed: builder-profile-entry
 *
 * State: ~/.rig/.transcript-ingest-state.json (LOCAL per ED1, never synced).
 * Secret scanning: gitleaks via lib/rig-memory-helpers#secretScanFile (D19).
 ...
 *
 * V1.5 NOTE: When `brain put_file` ships in the brain CLI (cross-repo P0 TODO),
 * transcripts will route to Supabase Storage instead of the page-write path.
 * Until then, all content rides `brain put <slug>` (stdin, YAML frontmatter for
 * title/type/tags); brain's native dedup keys on session_id.
 */
...
interface CliArgs {
   ...
   * default — the cross-machine boundary (rig-brain-sync, git push)
...
interface BulkResult {
  ...
  /**
   * D6: when set, indicates a process-level failure (brain CLI missing
   * or `brain import` crashed). Per-file errors (FILE_TOO_LARGE etc.)
...
const STATE_PATH = join(RIG_HOME, ".transcript-ingest-state.json");
...
function printUsage(): void {
  console.error(`Usage: rig-memory-ingest [--probe|--incremental|--bulk] [options]
...
function parseArgs(): CliArgs {
  ...
  let noWrite = process.env.RIG_MEMORY_INGEST_NO_WRITE === "1";
  let scanSecrets = process.env.RIG_MEMORY_INGEST_SCAN_SECRETS === "1";
...
function loadState(): IngestState {
  ...
  try {
    ...
    if (parsed.schema_version !== 1) {
      ...
      return { schema_version: 1, last_writer: "rig-memory-ingest", sessions: {} };
    }
    return parsed;
  } catch (err) {
...
function saveState(state: IngestState): void {
  ...
  // in rig-brain-sync.ts:saveSyncState.
  try {
...
function* walkrigArtifacts(ctx: WalkContext): Generator<{ path: string; type: MemoryType }> {
  const projectsRoot = join(RIG_HOME, "projects");

  // Eureka log: ~/.rig/analytics/eureka.jsonl
  const eurekaLog = join(RIG_HOME, "analytics", "eureka.jsonl");
  if (existsSync(eurekaLog) && ctx.args.sources.has("eureka")) {
    yield { path: eurekaLog, type: "eureka" };
  }

  // Builder profile: ~/.rig/builder-profile.jsonl
  const builderProfile = join(RIG_HOME, "builder-profile.jsonl");
...
function buildArtifactPage(path: string, type: MemoryType): PageRecord {
  ...
  // Extract repo slug from path: ~/.rig/projects/<slug>/...
  let slug_repo = "_unattributed";
  const m = path.match(/\/\.rig\/projects\/([^/]+)\//);
  if (m) slug_repo = m[1];
...
// ── Writer (batch via `brain import <dir>`) ───────────────────────────────
//
...
//   → snapshot ~/.brain/sync-failures.jsonl byte-offset           (D7)
//   → spawnSync `brain import <stagingDir> --no-embed --json`     (D6)
...
//
// We trust brain's content_hash idempotency (verified in
// ~/git/brain/src/core/import-file.ts:242-243, :478) — repeated imports
...
let _brainAvailability: boolean | null = null;
function brainAvailable(): boolean {
  if (_brainAvailability !== null) return _brainAvailability;
  try {
    // Probe `--help` for the `import` subcommand. brain v0.20.0+ ships
    ...
    // the whole stage with a confusing usage message from brain itself.
    // `brain --help` probes only CLI availability, not DB connectivity, so
    ...
    const help = execbrainText(["--help"], { timeout: 5000 });
    _brainAvailability = /^\s+import\s/m.test(help);
  } catch {
    _brainAvailability = false;
  }
  return _brainAvailability;
}
 ...
 *    existing frontmatter block before its close fence so brain's frontmatter
 ...
 *    this branch, artifact pages would land in brain with empty metadata.
 *
 * brain enforces slug = path-derived (slugifyPath in brain's sync.ts).
 * We do NOT set `slug:` in frontmatter — the staging-dir filename is the
 * source of truth and brain rejects mismatches.
 */
function renderPageBody(page: PageRecord): string {
  ...
  // (PR #1411) on the per-file `brain put` path; moved here so all staged
 ...
 *
 * D1: brain's `slugifyPath` (sync.ts:260) derives the slug from the
 ...
 * or rejected by brain's path-vs-frontmatter slug check (import-file.ts:429).
 *
 ...
 * mapping brain's failures back to sources and failed files get marked ingested.
 */
...
/**
 * Parse the `brain import --json` stdout payload (single JSON object on
 ...
 * which silently masked brain crashes as "0 imported, 0 failed = OK".
 */
...
/**
 * Read failures appended to ~/.brain/sync-failures.jsonl since the
 * snapshotted byte offset, and map them back to source paths.
 *
 * D7: brain import writes per-file failures to sync-failures.jsonl
 ...
 * staging-dir-relative filename brain saw (e.g. "transcripts/foo.md").
 ...
 *
 *   The actual cross-machine exfiltration boundary is `rig-brain-sync`,
 *   which runs a regex-based secret scanner on the staged diff before
 *   `git commit` (see bin/rig-brain-sync:78-110: AWS keys, GitHub
...
function preparePages(
  ...
  for (const { path, type } of walkAllSources(ctx)) {
    ...
    // because rig-brain-sync already gates the cross-machine boundary
...
/**
 * Make a per-run staging directory at ~/.rig/.staging-ingest-<pid>-<ts>/
...
function makeStagingDir(): string {
  const dir = join(RIG_HOME, `.staging-ingest-${process.pid}-${Date.now()}`);
 ...
 *
 * Instead of staging to ~/.rig/.staging-ingest-<pid>-<ts>/ and cleaning up
 * after `brain import`, remote-http users get a stable path that survives.
 * rig-brain-sync's allowlist pushes ~/.rig/transcripts/** to the
 * artifacts repo; the brain admin's pull job indexes them into the remote
 * brain. Local PGLite (if present) stays code-only.
 *
 * Path: ~/.rig/transcripts/<run-id>/  (run-id pid+ts so concurrent passes
...
/**
 * Detect whether the brain MCP is remote-http (Path 4) — and therefore we
 * should NOT call `brain import` because we don't want the local PGLite
 * polluted with transcripts (per plan D11).
 *
 * Reads ~/.claude.json directly (same fallback chain as rig-brain-detect
...
function isRemoteHttpMcpMode(): boolean {
  ...
  try {
    const parsed = JSON.parse(readFileSync(claudeJsonPath, "utf-8")) as {
      mcpServers?: {
        brain?: { type?: string; transport?: string; url?: string };
      };
    };
    const entry = parsed.mcpServers?.brain;
...
function cleanupStagingDir(dir: string): void {
  ...
  const verdict = checkOwnedStagingDir(dir, RIG_HOME);
  if (!verdict.ok) {
    console.error(
      `[brain] staging cleanup REFUSED: "${dir}" is not an owned staging dir ` +
...
/**
 * Track the currently-running brain import child + active staging dir so
 * SIGTERM/SIGINT on the parent process can:
 *   1. forward the signal to the child (otherwise brain orphans, holds the
 *      PGLite write lock, and burns CPU — observed during 2026-05-10 cold-run
 *      testing)
 *   2. PRESERVE the staging dir when brain has written an import-checkpoint
 *      pointing at it (the next /sync-brain run can resume from
...
/**
 * Returns true if brain has written ~/.brain/import-checkpoint.json with
  ...
  const forward = (signal: NodeJS.Signals) => () => {
    ...
    if (_activeStagingDir) {
      if (stagingDirIsCheckpointed(_activeStagingDir)) {
        ...
        // (in rig-brain-sync.ts) will see the checkpoint + dir and
        // re-invoke brain import against this same staging dir, picking
        ...
        // No checkpoint pointing here — the import never reached brain or
...
/**
 * Run brain import as an async child so we can install signal handlers
 ...
 */
/**
 * #1611: the `brain import` is the long pole on big brains. Its timeout is
 * configurable via RIG_INGEST_TIMEOUT_MS (default 30 min, 1min–24h) so large
 * memory corpora aren't SIGTERM'd mid-import. On timeout we SIGTERM the child,
 * which preserves brain's import-checkpoint.json (see installSignalForwarder)
...
export function resolveImportTimeoutMs(
  raw: string | undefined = process.env.RIG_INGEST_TIMEOUT_MS,
): number {
  if (raw === undefined || raw === "") return DEFAULT_IMPORT_TIMEOUT_MS;
  const n = Number.parseInt(raw, 10);
  if (!Number.isFinite(n) || Number.isNaN(n) || n < 60_000 || n > 86_400_000) {
    console.error(
      `[memory-ingest] RIG_INGEST_TIMEOUT_MS="${raw}" invalid (need 60000–86400000ms); using ${DEFAULT_IMPORT_TIMEOUT_MS}ms`,
    );
...
function runbrainImport(
  stagingDir: string,
  timeoutMs: number,
): Promise<{ status: number | null; stdout: string; stderr: string; timedOut: boolean }> {
  installSignalForwarder();
  return new Promise((resolve) => {
    // Seed DATABASE_URL from brain's own config so this stage works
...
async function ingestPass(args: CliArgs): Promise<BulkResult> {
  ...
  if (args.noWrite) {
    // --no-write: skip the brain import call but still record state for
    // prepared pages (treat them as ingested for dedup purposes). Matches
    // the prior contract from --help: "Skip brain put calls (still
    // updates state file)".
    ...
    state.last_writer = "rig-memory-ingest";
    saveState(state);
    return {
      written,
  ...
  if (!brainAvailable()) {
    const msg =
      "brain CLI not in PATH or missing `import` subcommand. Run /setup-brain.";
  ...
  // Phase 2: stage + (optionally) invoke brain import.
  //
  // Split-engine branch per plan D11: in remote-http MCP mode, we stage to a
  // PERSISTENT dir under ~/.rig/transcripts/ and SKIP `brain import`
  // entirely. rig-brain-sync push will pick the dir up via its allowlist
  ...
  // RIG_INGEST_RESUME_DIR (because brain's import-checkpoint.json points
  // at an existing dir from a prior SIGTERM'd run), reuse that staging dir
  // and skip the prepare/writeStaged phase entirely. brain's checkpoint
  // tells it where to resume.
  const remoteHttpMode = isRemoteHttpMcpMode();
  const resumeDir = process.env.RIG_INGEST_RESUME_DIR;
  // #1802 second entry point: this binary is runnable directly, so it must not
  // trust RIG_INGEST_RESUME_DIR just because it exists — a stale/poisoned env
  // could make us `brain import` (and later clean up) an arbitrary directory.
  ...
  const resuming = !remoteHttpMode
    ...
    && checkOwnedStagingDir(resumeDir, RIG_HOME).ok;
  if (!remoteHttpMode && resumeDir && resumeDir.length > 0 && !resuming) {
    console.error(
      `[memory-ingest] ignoring RIG_INGEST_RESUME_DIR="${resumeDir}" — not a proven staging dir (#1802); staging fresh.`,
    );
  }
  ...
  // either preserve (when brain checkpointed it) or synchronously clean up.
  ...
  try {
    ...
    if (resuming) {
      ...
      // brain's import will skip already-completed entries via its own
      ...
      // readNewFailures() can still map brain's per-file failures back to
    ...
    const syncFailuresPath = join(homedir(), ".brain", "sync-failures.jsonl");
    ...
    if (!args.quiet) {
      const action = remoteHttpMode
        ? "persisting to artifacts pipeline (skipping local brain import — remote-http mode)"
        : "running brain import";
      console.error(
        `[memory-ingest] staged ${staging.written} pages → ${stagingDir}; ${action}...`,
      );
    }

    // Remote-http branch (split-engine D11): no local brain import. The
    // staged markdown lives under ~/.rig/transcripts/<run-id>/ and the
    // next rig-brain-sync push will move it to the artifacts repo. From
    ...
    // and we have no per-page failures from brain to filter on. The
    ...
    if (remoteHttpMode) {
      ...
      state.last_writer = "rig-memory-ingest (remote-http mode)";
      saveState(state);
      if (!args.quiet) {
        console.error(
      ...
      // Skip the brain-import error handling + cleanupStagingDir paths
    ...
    // via brain's own pipelines. `--json` gives us structured counts.
    //
    // Async spawn (not spawnSync) so the signal forwarder installed in
    // runbrainImport propagates SIGTERM/SIGINT to the child. With sync
    // spawn, parent termination orphans the brain process (observed
    // during 2026-05-10 cold-run testing — brain kept running 15 min
    // after the orchestrator timed out).
    const importResult = await runbrainImport(stagingDir, resolveImportTimeoutMs());
    ...
    if (importResult.status !== 0) {
      // #1611/#1802 C3: on timeout, brain may have written
      // import-checkpoint.json so the next /sync-brain can resume. But an
      // INTERNAL timeout (runbrainImport kills the child and returns here)
      ...
      // only when brain actually checkpointed against this dir; otherwise let
      ...
      if (importResult.timedOut) {
        ...
        const msg = checkpointed
          ? `brain import timed out after ${mins}min; checkpoint preserved — re-run ` +
            `/sync-brain to resume (raise RIG_INGEST_TIMEOUT_MS for big brains)`
          : `brain import timed out after ${mins}min before writing a checkpoint; ` +
            `re-run /sync-brain to restage (raise RIG_INGEST_TIMEOUT_MS for big brains)`;
      ...
      const msg = `brain import exited ${importResult.status}: ${tail}`;
      console.error(`[memory-ingest] ERR: ${msg}`);
    ...
    if (!args.quiet) {
      // Echo brain's own progress lines on stderr through so the user sees
    ...
    if (importJson === null) {
      // brain exited 0 but didn't emit a parseable --json line. Treat as
      // ERR rather than silently passing zeros through — silent zeros let
      // a future brain-output regression mask data loss.
      const msg =
        "brain import exited 0 but emitted no parseable --json payload. " +
    ...
    // Phase 3: state recording. Only files that landed in brain get
    // their mtime+sha256 stamped. Failed source paths are deliberately
    // left un-state'd so the next run re-prepares them and brain's
    ...
    if (!args.quiet) {
      console.error(
        `[memory-ingest] brain import: ${importJson.imported ?? 0} imported, ` +
          `${importJson.skipped ?? 0} unchanged, ${importJson.errors ?? 0} failed` +
          (failedSources.size > 0
            ? ` (see ~/.brain/sync-failures.jsonl for details)`
            : ""),
      );
    }
  } finally {
    // #1802 D1: in remote-http mode `stagingDir` is the PERSISTENT transcript
    // dir (makePersistentTranscriptDir, under ~/.rig/transcripts/) that
    // rig-brain-sync push must pick up — it is NOT a `.staging-ingest-*` dir
...
async function main(): Promise<void> {
  ...
  // Engine tier detection — informational; routing happens in brain server-side.
...
// The orchestrator runs it as `bun rig-memory-ingest.ts ...`, where
// import.meta.main is true, so the CLI path is unaffected.
if (import.meta.main) {
  main().catch((err) => {
    console.error(`rig-memory-ingest fatal: ${err instanceof Error ? err.message : String(err)}`);
    process.exit(1);
  });
}
