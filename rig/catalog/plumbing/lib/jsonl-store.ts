/**
 * jsonl-store — shared, audited plumbing for rig's append-only JSONL stores.
 *
 ...
 *      down the whole read).
 *
 * Extracted from `bin/rig-learnings-log` (D2A) so `rig-learnings-*` and the
 * new `rig-decision-*` bins share ONE audited path — a new injection pattern or
