/**
 * bin-context — tiny shared helpers for non-interactive rig bins that need the
 * project slug, current branch, and argv flags. Extracted from the decision bins
 * (rig-decision-log / rig-decision-search) so the slug/branch/flag plumbing
 * lives in one audited place instead of being copy-pasted per bin.
 */

import { spawnSync } from "child_process";

/** Resolve the project slug via the `rig-slug` helper (parses `SLUG=...`). */
