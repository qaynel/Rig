 ...
 * Conductor's RIG_-prefixed forms are present. Promote the RIG_ form to
 * canonical when canonical is empty, so subprocesses (brain embed,
...
/**
 * Pure form: returns a copy of `base` with each RIG_-prefixed key promoted
...
export function promotedEnv(base: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  for (const key of PROMOTED_KEYS) {
    if (!out[key] && out[`RIG_${key}`]) {
      out[key] = out[`RIG_${key}`];
    }
  }
  return out;
}
