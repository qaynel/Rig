 ...
 *
 * Single source of truth shared by `lib/redact-engine.ts`, `bin/rig-redact`,
 * `bin/rig-redact-prepush`, and (via `scripts/resolvers/redact-doc.ts`) the
...
// keys like AKIA-EXAMPLE-KEY are bare tokens, so the guard still catches them.
const PLACEHOLDER_SUBSTRING = [
  /example/i, // AKIA-EXAMPLE-KEY etc — AWS docs convention
  /^changeme$/i,
  /^redacted/i,
  /^placeholder/i,
  /^dummy/i,
...
export const PATTERNS: RedactPattern[] = [
  ...
  },
  {
    ...
    // rig drives glab first-class — these were a coverage gap (#1946).
