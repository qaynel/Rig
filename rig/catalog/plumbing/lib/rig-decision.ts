/**
 * rig-decision — event-sourced institutional decision memory.
 *
 ...
 * text can't carry instructions. brain is never required — this is the reliable
...
/** Resolve the per-project decision store paths. Bins pass slug + RIG_HOME. */
export function decisionPaths(slug: string, rigHome?: string): DecisionPaths {
  const home = rigHome || process.env.RIG_HOME || join(homedir(), ".rig");
