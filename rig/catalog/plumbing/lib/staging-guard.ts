/**
 * staging-guard — fail-closed ownership proof for rig ingest staging dirs.
 *
 * Fixes #1802. The /sync-brain memory stage stages prepared pages to a
 * throwaway dir under ~/.rig and `rm -rf`s it when done. The resume path
 * (#1611) reused brain's `import-checkpoint.json` `dir` field as that staging
 * dir WITHOUT proving it was one. A poisoned checkpoint — `dir` = the repo
 * root, written when an autopilot `brain import` was SIGTERM'd while CWD was
 ...
 * proved it owned. This module is the single definition of "a path rig is
 ...
 *   2. Structural    — canonical path is a DIRECT child of $RIG_HOME named
 ...
 *                      git working tree.
 *   4. Minted by us  — a `.rig-staging` marker file (written by
 ...
 *
 * The deeper, "inevitable" fix lives upstream in brain: checkpoint.dir should
 * always be a brain-minted staging dir, never CWD. This guard is the
 * mitigation at rig's own rm -rf boundary; see the companion brain issue.
 */
...
export const STAGING_MARKER = ".rig-staging";
...
/**
 * Prove (fail-closed) that `dir` is a rig-owned ingest staging directory
 ...
 *
 * @param dir         Candidate path (e.g. brain checkpoint.dir, or the active staging dir).
 * @param rigHome  Resolved $RIG_HOME (injected for testability).
 */
export function checkOwnedStagingDir(dir: string, rigHome: string): StagingVerdict {
  ...
  if (dirname(canon) !== home) {
    return { ok: false, reason: `not a direct child of RIG_HOME (${home})` };
  }
...
export function isOwnedStagingDir(dir: string, rigHome: string): boolean {
  return checkOwnedStagingDir(dir, rigHome).ok;
}
