/**
 * brain-guards — defense-in-depth against brain's destructive code paths (#1734).
 *
 * brain (the separate CLI rig shells out to) can rm-rf a user's working tree
 * during an autopilot race (its own bug, upstream brain #1526). rig can't fix
 * that, but it MUST stop treating brain's destructive subcommands as safe. These
 ...
 *
 * Design notes grounded in the real brain 0.41.x surface:
 ...
 *   - The autopilot lock filename isn't documented and (brain #1226) ignores
 *     BRAIN_HOME, so the live `brain autopilot` process is the PRIMARY signal;
 *     known lock paths under both the configured home and ~/.brain are secondary.
 ...
 *   - Path containment uses realpath so a symlink inside ~/.brain/clones can't
...
import { execbrainJson, execbrainText, NEEDS_SHELL_ON_WINDOWS } from "./brain-exec";
import { parseSourcesList, type brainSourceRow } from "./brain-sources";

export function brainHome(env: NodeJS.ProcessEnv = process.env): string {
  return env.BRAIN_HOME || join(homedir(), ".brain");
}

/**
 * Directories brain owns and may delete safely. A source whose local_path
 * resolves inside one of these is brain-managed; outside = user-managed and
 * must be protected. Both the configured home and the default ~/.brain are
 * checked because brain #1226 shows home-resolution is inconsistent.
 */
function clonesDirs(env: NodeJS.ProcessEnv = process.env): string[] {
  return [...new Set([join(brainHome(env), "clones"), join(homedir(), ".brain", "clones")])];
}
...
/**
 * Detect a running brain autopilot. Refuse the caller's destructive op only on
...
export function detectAutopilot(
  ...
  // Secondary signal: known lock files. brain #1226 — the lock ignores
  // BRAIN_HOME, so check both the configured home and the default ~/.brain.
  const lockPaths = probe.lockPaths ?? [
    join(brainHome(env), "autopilot.lock"),
    join(homedir(), ".brain", "autopilot.lock"),
    join(brainHome(env), "autopilot.pid"),
    join(homedir(), ".brain", "autopilot.pid"),
  ];
  ...
  // Primary signal: a live `brain autopilot` process.
  const running = (probe.processRunning ?? defaultProcessRunning)();
  if (running) return { active: true, signal: "process:brain autopilot" };
...
// per process (keyed to the resolved brain identity) rather than persisting a
...
export function brainSupportsKeepStorage(env: NodeJS.ProcessEnv = process.env): boolean {
  const key = brainIdentity(env);
  if (_keepStorageMemo && _keepStorageMemo.key === key) return _keepStorageMemo.value;
  let value = false;
  for (const args of [["sources", "remove", "--help"], ["--help"]]) {
    try {
      if (/--keep-storage/.test(execbrainText(args, { baseEnv: env, timeout: 5_000 }))) {
        value = true;
        break;
      }
    } catch {
 ...
 */
export function fetchSources(env: NodeJS.ProcessEnv = process.env): brainSourceRow[] {
  const raw = execbrainJson(["sources", "list", "--json"], { baseEnv: env });
  if (raw === null) throw new Error("brain sources list returned no JSON");
 ...
 *   - the row is user-managed (remote_url set AND local_path outside brain's
 *     clones) and brain has no --keep-storage to protect the files.
 *
 * Allowed: absent row (no-op), brain-managed (inside clones), or path-managed
 * without a remote_url (brain's remove won't touch an outside-clones path that
...
export interface DecideRemoveOpts {
  ...
  fetchRows?: (env: NodeJS.ProcessEnv) => brainSourceRow[];
}
...
export function decideSourceRemove(
  ...
  const keepStorage = opts.keepStorage ?? brainSupportsKeepStorage(env);
  const extra = keepStorage ? ["--keep-storage"] : [];

  let rows: brainSourceRow[];
  try {
  ...
  if (userManaged) {
    ...
    return {
      ...
      reason:
        ...
        `${row.local_path} outside brain clones) — this brain has no --keep-storage to ` +
        `protect the working tree. Upgrade brain or remove the source manually.`,
    };
  }

  return { allow: true, extraArgs: extra, reason: "brain-managed or path-managed without remote_url" };
}
 ...
 *
 * A source with a remote_url can trigger brain's auto-reclone, the ungated
 * rm-rf path behind the data loss (brain #1526). Require an explicit
...
export function decideCodeSync(
  ...
  env: NodeJS.ProcessEnv = process.env,
  allowReclone = false,
  fetchRows: (env: NodeJS.ProcessEnv) => brainSourceRow[] = fetchSources,
): SyncDecision {
  let rows: brainSourceRow[];
  try {
    rows = fetchRows(env);
  ...
  if (row?.config?.remote_url && !allowReclone) {
    return {
      ...
      reason:
        ...
        `delete the working tree. Re-run /sync-brain with --allow-reclone to proceed.`,
    };
  }
