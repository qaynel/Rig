/**
 * rig-memory-helpers — shared helpers for the V1 memory ingest + retrieval pipeline.
 *
 * Imported by:
 *   - bin/rig-memory-ingest.ts (Lane A)
 *   - bin/rig-brain-sync.ts   (Lane B)
 *   - bin/rig-brain-context-load.ts (Lane C)
 ...
 *   ED1 — engine-tier cache lives in ~/.rig/.brain-engine-cache.json (60s TTL)
 *
 * NOTE: secretScanFile() currently shells out to `gitleaks` from PATH; the vendored
 * binary install is part of Lane E (setup-brain). When gitleaks is missing, the
...
export interface brainManifestQuery {
  id: string;
...
export interface brainManifest {
  schema: number; // brain.schema in frontmatter; V1 = 1
  context_queries: brainManifestQuery[];
}
 ...
 * (per ED1 — brain-side session_id dedup uses repo as a tag).
 *
 * Examples:
 *   https://github.com/garrytan/rig.git → github.com/garrytan/rig
 *   git@github.com:garrytan/rig.git     → github.com/garrytan/rig
...
function gitleaksAvailable(): boolean {
  ...
  try {
    ...
    process.stderr.write(
      "[rig-memory-helpers] gitleaks not in PATH; secret scanning disabled. " +
      "Run /setup-brain to install (or `brew install gitleaks`).\n"
    );
  }
 ...
 * Replaces the inadequate regex scanner in bin/rig-brain-sync (which only
...
function rigHome(): string {
  return process.env.RIG_HOME || join(homedir(), ".rig");
}

function engineCachePath(): string {
  return join(rigHome(), ".brain-engine-cache.json");
}

function errorLogPath(): string {
  return join(rigHome(), ".brain-errors.jsonl");
}

/**
 * Detect which brain engine is active (PGLite vs Supabase) and cache the
 * answer for 60s in ~/.rig/.brain-engine-cache.json. Caching avoids
 * fork+exec'ing `brain doctor --json` on every skill start.
 *
...
export function detectEngineTier(): EngineDetect {
  ...
  try {
    ...
    writeFileSync(
      engineCachePath(),
      JSON.stringify({ ...fresh, last_writer: "rig-memory-helpers.detectEngineTier" }, null, 2),
      "utf-8"
    );
  } catch {
...
// Returns brain's config.json path, honoring BRAIN_HOME env var with a
// fallback to ~/.brain. brain >=0.25 dropped the top-level `engine` field
// from doctor output, so this file is the only reliable source for engine
// detection on that version. See #1415.
function brainConfigPath(): string {
  const root = process.env.BRAIN_HOME || join(homedir(), ".brain");
  return join(root, "config.json");
}

// Best-effort JSONL append to ~/.rig/.brain-errors.jsonl. Never throws.
function logbrainError(kind: string, detail: string): void {
  try {
...
function freshDetectEngineTier(): EngineDetect {
  ...
  try {
    ...
    // object. brain doctor exits 1 whenever health_score < 100, which is
    ...
    try {
      ...
      logbrainError("doctor_parse_failure", String(parseErr));
    }
  }
  ...
  // brain >=0.25 ships schema_version:2 doctor output which dropped the
  // top-level `engine` field. Fall back to brain's config.json (respects
  // BRAIN_HOME). "supabase" here means "remote postgres" — brain config
  ...
  if (engine === "unknown") {
    try {
      const cfg = JSON.parse(readFileSync(brainConfigPath(), "utf-8"));
      if (cfg?.engine === "pglite") engine = "pglite";
      else if (cfg?.engine === "postgres" || cfg?.database_url) engine = "supabase";
    } catch (cfgErr) {
      logbrainError("config_read_failure", String(cfgErr));
    }
  }
...
/**
 * Parse the `brain:` section out of a SKILL.md.tmpl frontmatter block.
 ...
 * runtime read path used by rig-brain-context-load; it tolerates extra
 * fields and relies on validation having already happened upstream.
 */
export function parseSkillManifest(skillFilePath: string): brainManifest | null {
  if (!existsSync(skillFilePath)) return null;
  const content = readFileSync(skillFilePath, "utf-8");
  const frontmatter = extractFrontmatter(content);
  if (!frontmatter) return null;
  const brain = extractbrainBlock(frontmatter);
...
function extractbrainBlock(frontmatter: string): brainManifest | null {
  // Naive YAML extraction — finds the `brain:` key and parses its sub-tree.
  // Real YAML parsing avoided to keep zero-deps; gen-skill-docs validates the
  // shape strictly at build time.
  const lines = frontmatter.split("\n");
  const start = lines.findIndex((l) => /^brain\s*:/.test(l));
  if (start === -1) return null;

  // Collect indented lines under `brain:` until next top-level key or EOF
  const block: string[] = [];
  ...
  const queries: brainManifestQuery[] = [];
  ...
  if (cqMatch) {
    ...
    for (const item of items) {
      const q: Partial<brainManifestQuery> = {};
      ...
      if (q.id && q.kind && q.render_as) {
        queries.push(q as brainManifestQuery);
      }
    }
  }
...
const ERROR_LOG_PATH = join(rigHome(), ".brain-errors.jsonl");

/**
 * Wrap an op with structured error logging. Logs success/failure + duration
 * to ~/.rig/.brain-errors.jsonl for forensic debugging. Replaces ad-hoc
