#!/usr/bin/env node
// rig — shared configuration resolver
...
function isDeactivationCommand(text) {
  ...
  return t === 'stop rig' || t === 'normal mode';
}
...
function getConfigDir() {
  if (process.env.XDG_CONFIG_HOME) return path.join(process.env.XDG_CONFIG_HOME, 'rig');
  if (process.platform === 'win32') {
    return path.join(process.env.APPDATA || path.join(os.homedir(), 'AppData', 'Roaming'), 'rig');
  }
  return path.join(os.homedir(), '.config', 'rig');
}
...
function getDefaultMode() {
  const envMode = process.env.RIG_DEFAULT_MODE;
