...
- [ ] **RIG-133 — The signature freezes 68 samples; it should freeze the properties and a case generator**
	**Status:** OPEN (2026-08-25, re-confirmed 2026-08-26) — GitHub #42 — no progress; `tests/advanced-oracle.test.js` still 65 enumerated cases; blocks RIG-125's last step; awaiting owner approve-for-Coding · [Solution](tickets/RIG-133.md)
...
- [ ] **RIG-142 — spawnTask safety defaults (shell:false, env allowlist) are caller-overridable with no guard or test**
	**Status:** OPEN (2026-08-27) — GitHub #97 — `spawnTask`'s `shell:false` and env allowlist are both placed before `...options`, so any caller can silently override them; no assertion or test prevents this; structural gap in AT-LF-21/AT-LF-22 enforcement · [Solution](tickets/RIG-142.md)
	**Class:** CORRECTNESS / SHELL TRUST.
- [ ] **RIG-141 — runReadOnly hard-refuses all tasks when sandbox absent, including network-granted ones; granted-network path untested**
	**Status:** OPEN (2026-08-27) — GitHub #96 — when `unshare`/`sandbox-exec` is absent `runReadOnly` returns early for all tasks including `cmd.network === true` ones; over-blocks beyond AT-LF-22 scope; the granted-network execution path has no test at all · [Solution](tickets/RIG-141.md)
	**Class:** CORRECTNESS / SHELL TRUST.
...
- [ ] **RIG-115 — Author lint-format acceptance for applicability, execution consent, and shell trust**
	**Status:** IMPLEMENTING (2026-08-26) — GitHub #64 — oracle re-signed; `AT-LF-20`, `AT-LF-21`, and `AT-LF-23` implemented; `AT-LF-22`/`24` pending · [Solution](tickets/RIG-115.md)
	**Blocker:** Two of the five signed shell-trust cases are still unimplemented (`AT-LF-22` network denial, `AT-LF-24` symlink refusal). `AT-LF-20` single-use approval, `AT-LF-21` filesystem/env isolation, and `AT-LF-23` resource/time-cap kill-and-report have landed.
	**Acceptance:**
