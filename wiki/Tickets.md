...
- [ ] **RIG-115 — Author lint-format acceptance for applicability, execution consent, and shell trust**
	**Status:** IMPLEMENTING (2026-08-26) — GitHub #64 — oracle re-signed; `AT-LF-21` and `AT-LF-23` implemented; `AT-LF-20`/`22`/`24` pending · [Solution](tickets/RIG-115.md)
	**Blocker:** Three of the five signed shell-trust cases are still unimplemented (`AT-LF-20` single-use approval, `AT-LF-22` network denial, `AT-LF-24` symlink refusal). `AT-LF-21` filesystem/env isolation and `AT-LF-23` resource/time-cap kill-and-report have landed.
	**Acceptance:**
