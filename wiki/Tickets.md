...
- [x] **RIG-108 — Resolve the four zero-caller runtime modules (wire or delete)**
	**Status:** RESOLVED (2026-08-24) — full gate green, ready to commit · [Solution](tickets/RIG-108.md)
	**Landed:** All runtime modules now have production callers guarded by a repository-wide caller test. The existing enforcement, release-review, and commit-dispatch seams are proven; catalogue apply now uses managed graft blocks with journaled legacy migration and exact named-block uninstall.
