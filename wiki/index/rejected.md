...
| Rejected | Why |
|---|---|
| A RAG / embedding pipeline over the wiki | Considered 2026-09-05 as the mechanism for "active references". Turned down on four grounds. (1) Scale: 194 traces with structured frontmatter is a `WHERE` clause, not a retrieval problem — `scripts/build-wiki-index.js` is 126 lines and already parses the frontmatter a query would filter on. (2) Determinism: RAG returns *semantically similar*, the consultation index needs *authoritative* ("these entries govern this path"). Non-deterministic retrieval under a product whose thesis is a signed, unforgeable oracle is off-thesis. (3) Install cost: an embedding index means a runtime, a model dependency, and mutable state in every installed repository. (4) **Decisive: RAG removes deletion pressure.** The root cause on record is that nothing forces consumption to bound production. Making retrieval cheap makes bloat free to tolerate, curing the symptom while entrenching the disease. Revisit only if the corpus passes ~1,000 documents *and* queries become genuinely semantic rather than path- or topic-scoped. |
| "Trim the wiki by writing a better map" | Already attempted. PR #146 (`wiki-2026-09-04-overhaul-and-maintenance`, merged `e5f62c63`) was **+2,470 / -136** across 68 files — a maintenance pass that added seventeen lines for every one it removed. A hub carrying 65 outbound links (`topics/onboarding-flow.md`) is a second unbounded read, not a reference. The map is subject to the same write/read asymmetry that produced the traces, so improving it cannot fix it. See [measurements](../reasoning/2026-09-05-overhaul-scope-measurements.md). |
| Renaming vendored catalogue files to `rig-*` | Proposed 2026-09-05 as part of the overhaul. `rig/catalog/skills/` is 6.9M and ships `LICENSE.upstream` and `UPSTREAM.md` at its root; the tree declares its own provenance. A rename re-badges third-party work as first-party, keeps the maintenance burden, and discards the ability to pull upstream fixes. The filed direction is the inverse — reference, do not vendor. |
