...
| 2026-08-27 | **Guarantees are not split into independent branches without a call-site/conjunct list.** RIG-115's per-AT-LF branches each passed their own test while leaving the guarantee false (unpersisted approval, one guarded execution path of two, half of an "or" requirement). Named as [guarantee sharding](../mistakes/guarantee-sharding.md); a new `mistakes/` directory holds this and future named anti-patterns, checked at `rig-grilling` (enumerate the list before splitting) and `rig-execution` (re-check the seam before a completion claim). [RIG-120 / #68](https://github.com/qaynel/Rig/issues/68) must not treat RIG-115 as Done until the guarantee holds at every call site, not just the tested slice. | [reasoning trace](../reasoning/2026-08-27-guarantee-sharding-mistake.md) |
| 2026-08-27 | **RIG-138/139/140 closed by a consolidated fix on `rig/lib/lint-format.js`, not by merging any `rig-115-at-lf-*` sibling branch.** Durable one-use approval, `runGrade`+`runReadOnly` symlink/cwd containment, and a memory ceiling (separate-process RSS watchdog, `rig/lib/memory-guarded-exec.js`, kept synchronous to avoid breaking the frozen `tests/advanced-oracle.test.js`) all landed together, proven by the previously-red `tests/guarantee-coverage.test.js` (AT-PROC-1a/b/c) going green with the full gate passing. [RIG-138 / #93](https://github.com/qaynel/Rig/issues/93), [RIG-139 / #94](https://github.com/qaynel/Rig/issues/94), [RIG-140 / #95](https://github.com/qaynel/Rig/issues/95) closed. | [reasoning trace](../reasoning/2026-08-27-rig138-139-140-shell-trust-fix.md) |
| 2026-08-28 | **`runGrade` now mirrors `runReadOnly`'s per-command network isolation and refusal contract.** Ungranted commands are prefixed with the verified OS isolation mechanism; if the host cannot provide it, that command reports `network_isolation_unavailable`. Explicit `network: true` commands bypass the prefix and still run, including when the sandbox executable is unavailable. The change closes the AT-LF-22 call-site/conjunct gap without changing the frozen oracle or requiring a re-sign. [RIG-143] | [reasoning trace](../reasoning/2026-08-28-runGrade-network-isolation-grilling.md) |
...
| 2026-09-01 | **Pre-approval skill projection drops to eight on the adaptive install only.** The Path B (`--with-runtime`) install projects only the eight mandatory skills into host discovery and stages the complete 55-skill shelf under `.rig/runtime/rig/catalog/skills/`, where no host discovers it. The legacy markdown-only Tier 1 install keeps its full native fan-out, gated by `default_delivery`, so RIG-149's "the vendored router is addressable as `rig`" guard survives where it still applies; slice 6 retires that path as a whole rather than hollowing it out now. | [slice 1 trace](../reasoning/2026-09-01-path-b-slice1-catalogue.md), [onboarding flow](../topics/onboarding-flow.md) |
| 2026-09-01 | **The skill shelf is a generated, digest-pinned catalogue with no alias fiction.** Family/capability membership is self-declared frontmatter compiled by `scripts/build-skill-catalog.js` into `rig/catalog/skills/catalog.json`, checked for drift in `npm test`, and pinned byte-for-byte at `.rig/catalog.json`. `migrations.json` ships with an empty alias map because no callable name actually changed: `connect-chrome` now declares the name its collision tie-break already produced, removing the collision at the source instead of aliasing around it. | [slice 1 trace](../reasoning/2026-09-01-path-b-slice1-catalogue.md), [the catalogue](../topics/the-catalogue.md) |
| 2026-08-31 | **Path B is agent-led graft, not a skill picker and not a Rig-code inference engine.** The onboarding host agent receives full repo context plus the Rig capability catalogue (organised into families), decides relevance, reuses/extends existing infra, grafts missing Rig behaviour, and escalates only critical decisions. The user reviews the resulting improvement. Repo-specific state lives under `.rig/`. Payload size warns; duplicate-write fails. Router re-test does not block. D24 still constrains what *Rig code* infers; the consent UX shifts from default-full family-trim to agent-proposed graft plus summary approval. | [product direction](../reasoning/2026-08-31-path-b-product-direction.md), [adapt scope](../reasoning/2026-08-31-path-b-adapt-scope.md), [vertical slice](../reasoning/2026-09-01-path-b-slice5-vertical.md) |
| 2026-09-01 | **Path B now has a public operator path and closed check surface.** `install rig` maps repeatable host selections through the existing registry, always installs the active runtime, and stops at explicit onboarding approval. The shared check reports journal-backed file/byte growth as warnings and known ownership, reference, projection, and state regressions as hard failures. | [operator and check closure](../reasoning/2026-09-01-path-b-slice6-operator-check.md) |
...
| D11 | 2026-07-28 | Removal is part of the product. Managed blocks, install manifest, preimages. *Clarified 2026-08-28: the editable journal names candidates but is not independent proof of ownership; common CI pipelines are preserved, and CI line-strip is limited to the uniquely attributable GitHub pointer.* | [install manifest and removal](../topics/install-manifest-removal.md) |
...
| GA-37 | 2026-08-26 | Pins `GA-26`'s untrusted-task principle to five concrete, testable guarantees for every lint-format repository task, even under `shell: false`: single-use plan-bound approval; filesystem/env isolation, including through symlinks; default-deny network reachability; killed-and-reported memory/wall-clock caps; refused symlink escapes. Closes RIG-115's shell-trust suite (`AT-LF-20`–`AT-LF-24`). |
| GA-38 | 2026-08-29 | Closes RIG-144's capability-boundary gap, superseding "ship + waiver": undeclared network capability is a distinct, visibly-flagged temporary compatibility state (not equivalent to an explicit grant), shaped so a later release can flip it to deny without a schema change; the installed check runner has exactly one canonical implementation (`rig/lib/check-runner.js`), with `.rig/bin/check.js` a generated/copied artifact rather than a second hand-maintained source — a drift test alone was rejected as detecting the defect class instead of removing it; the CI/non-interactive runner grants capability beyond its default only from committed policy, never an implicit or ephemeral approval, structured so a future interactive one-use grant reuses the same evaluator; timeout and memory ceilings are per-binding configurable defaults (10 min / 2 GiB), not hard semantic limits, and timeout kill covers the whole descendant process tree. AD-39 records the separate committed policy file and exact-grant schema. |
...
| GA-37 | `GA-26` names the untrusted-task controls Rig must enforce—policy, least privilege, secret isolation, network restriction, resource/time limits—but the existing acceptance set (`AT-LF-5`) only discloses that boundary, it does not pin what any control concretely guarantees; RIG-115's reconciliation found session lifetime, filesystem/env isolation, memory limits, and symlink handling still labeled assumptions, with network denial requested but unenforced. Five concrete guarantees close the gap: a plan approval authorizes exactly one execution of its exact digest and does not carry over to a later run; a task's working directory and every path it touches must resolve inside the repository even through a symlink, and it receives no ambient environment variables beyond an explicit allowlist; a task has no outbound network reachability unless the plan explicitly allows it; a task that exceeds a configured memory ceiling or wall-clock timeout is killed and reported as its own distinct non-passing state (`GA-33`); a repository-supplied symlink resolving outside the repository is refused like any other escape attempt, never followed. None of the five was an implementing agent's call to make, since each is a safety promise rather than a mechanism detail; the intent owner approved all five as proposed. |
| GA-38 | RIG-144's installed CI/git-floor runner gets the same class of guarantee `GA-37` gave `lint-format`, plus a structural fix the discovery process itself exposed. Network capability is three states, not two: explicit `none` and explicit `required` are enforced immediately; an undeclared binding keeps running (avoiding a silent breakage of the ~115 existing catalog packs) but must emit a visible diagnostic and be recorded as its own distinct state, never folded into "allowed," so a later release can flip undeclared to deny without a schema change. The installed runner (`rig/catalog/baseline/check.js`, materialized to `.rig/bin/check.js`) and the interactive runner (`rig/lib/checks.js`) may no longer each carry their own copy of the containment/execution logic — a fix landing in one and silently missing the other is the exact defect this ticket's own investigation found; both now require one canonical module (`rig/lib/check-runner.js`), with the installed copy remaining a materialized artifact rather than a second hand-edited source. A binding's committed policy, not an implicit or ephemeral approval, is the only way to authorize a capability beyond its default on this non-interactive path; refusal is the alternative, never silent acquisition, structured so a later interactive one-use-grant path reuses the same evaluator. Timeout and memory ceilings are mandatory on every command, at configurable per-binding defaults (10 min / 2 GiB) rather than fixed semantic limits, with timeout kill covering the whole descendant process tree. None of this was an implementing agent's call; the intent owner approved the shape and corrected three points in the draft before it was built. |
