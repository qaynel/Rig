...
looks contained. `AT-LF-21` is implemented in `runReadOnly` (contained cwd,
allowlisted env); `AT-LF-23` is implemented (timeoutMs kill-and-report);
`AT-LF-20`/`AT-LF-22`/`AT-LF-24` remain unimplemented, frozen ahead of the
runtime that must satisfy them.
