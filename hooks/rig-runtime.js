...
const { getClaudeDir } = require('./rig-config');

const STATE_FILE = '.rig-active';
...
// Live mode written by activate/mode-tracker. Absent flag = rig off.
function readMode() {
  try {
...
function writeHookOutput(event, mode, context = '') {
  ...
  if (isCodex) {
    const output = { systemMessage: `RIG:${mode.toUpperCase()}` };
    if (context) {
